...
public class PaUserPaperServiceImpl implements PaUserPaperService{
	...
	public void setPaUserPaperDao(PaUserPaperDao paUserPaperDao) {
		this.paUserPaperDao = paUserPaperDao;
	}
	...
	public PaUserPaper add(PaUserPaper paUserPaper) {
    	...
    	Date createDate = paUserPaper.getCreateDate();
    	if(createDate == null) {
    		createDate = new Date();
    	}
    	paUserPaper.setCreateDate(createDate);
    	paUserPaper.setModifyDate(createDate);
	...
	public PaUserPaper modify(PaUserPaper paUserPaper) {
    	...
    	Date modify = paUserPaper.getModifyDate();
    	paUserPaper.setModifyDate(modify != null ? modify : new Date());
	...
	public void remove(PaUserPaper paUserPaper) {
		try {
			...
			if(log.isDebugEnabled()) {
				log.debug("删除数据库无存在ID为 {} ,异常为：{}", paUserPaper.getId(), e);
			}
		}
	}
