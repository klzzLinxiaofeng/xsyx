   ...
   return t ;
    ...
    $(function () {
        ...
        $.each(items, function (index, n) {
            $('.paper_list').append('<div class="sj_view"></div>')
            var data=eval(items[index]);
            console.log(data.body.group);
            $('<div class="paper_title">\n' +
                '        <div class="p1"><span class="title">'+data.paper.title+'</span>(满分<span class="manfen">'+data.paper.score+'</span>分)</div>\n' +
                '        <div class="p2">\n' +
                '            <ul>\n' +
                '            \t<li class="xs"><b></b>学生：<span>'+data.studentInfo.studentName+'</span></li>\n' +
                '                <li class="zf"><b></b>班级：<span>'+data.studentInfo.teamName+'</span></li>\n' +
                '                <li class="bh"><b></b>得分：<span>'+data.studentInfo.studentScore+'</span></li>\n' +
                '                <li class="tm"><b></b>用时：<span>'+secondToDate(data.studentInfo.sumTime)+'</span></li>\n' +
                '            </ul>\n' +
                '        </div>\n' +
                '    </div>').appendTo($(".sj_view").eq(index));
            ...
            $(group).each(function(index){
                ...
                $("<div class='test-paper-right-content' style='padding: 0;'><p style='font-weight: bold;'>"+group[index].pos+"、"+group[index].name+"</p><p>"+group[index].memo+"</p><ul>"+li+"</ul></span>").appendTo($(".sj_view").eq(_index));
