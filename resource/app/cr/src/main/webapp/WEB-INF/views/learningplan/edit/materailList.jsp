	...
	function chose(dataType){
	    ...
	    if(dataType == "myresource"){//我上传的资源
	    	$("#personSign").val(dataType);
	 		url = url + "/learningPlan/materail/myList?index=index&resType=2&personType=res_person";
	    }else if(dataType == "favresource"){//我收藏的资源
	    	$("#personSign").val(dataType);
	    	url = url + "/learningPlan/materail/myList?index=index&resType=2&personType=fav_resource";
	    }else if(dataType == "myshare"){//我的共享
	    	$("#personSign").val(dataType);
	    	url = url + "/learningPlan/materail/myList?index=index&resType=2&personType=res_share";
	    }
    ...
    function changeMicroTab(type) {
        ...
        if (type == "myMicro") {
        	...
        	$("#sign").val("school");
       		...
       		url = url + "/learningPlan/resource/list?personType=res_school&unitId=${unitId}&resourceType=2";
			source = "来自校本资源库";
		...
		$.ajax({
			...
			success : function(data) {
				...
				for(var i=0; i<resources.length; i++) {
                    ...
                    '</a></dt><dd><div class="item-msg"><div class="item-title"><span class="'+resource.iconClass+' icon-file res-iconb"></span>'+
                    ...
                    '<a href="javascript:void(0)" onclick="previewMicro(\''+resource.id+'\')" title="'+resource.title+'">'+resource.title+'</a></span></div><span class="i1">课件目录：'+resource.catalogName+'</span>'+ 
                    ...
                    if(deleteMicro==1 && resources.length==1) {
                    	...
                    	html += '<button class="btn btn-danger" data-file_id='+resource.lpFileId+' data-type="4" onclick="addMaterail('+unitId+',1)">更换</button></div></div></dd></dl>';
                    } else {
                    	html += '<button class="btn btn-danger" data-file_id='+resource.lpFileId+' data-type="4" onclick="lpUnitFileDelete('+resource.lpFileId+',this,'+unitId+',\'你确定要删除本课件吗？\')">删除</button></div></div></dd></dl>';
                    }
				}
				var num = 0;
				if(deleteMicro==1) {
					...
					var button = '<button class="btn btn-danger" data-file_id='+file_id+' data-type="4" onclick="lpUnitFileDelete('+file_id+',this,'+unitId+',\'确定删除该微课吗？\')">删除</button>';
