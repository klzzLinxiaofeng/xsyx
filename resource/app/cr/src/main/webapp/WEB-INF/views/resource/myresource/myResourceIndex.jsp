...
<%@page import="platform.education.commonResource.web.resource.contans.ResResourceType"%>
				...
				<div class="weike_top">
					<div class="gjz_select">
						...
						<button onclick="search();" class="btn btn-primary">搜索</button>
							<a href="${pageContext.request.contextPath}/resource/uploadIndex?resType=${resType}&resourceType=${personType}&dm=${dm}" class="upload">
								...
								<div class="rs_platformContainer_1">
									<div class="main" style="margin-left: -1px;">
										...
										<jsp:include page="/views/embedded/jqpagination.jsp"
											...
											<jsp:param name="url" value="/resource/myResource?resType=${resType}&personType=${personType}" />
...
<script type="text/javascript">
	...
	function changeType(resType,obj) {
		...
		$(obj).addClass("on");
		var data={"resType":resType,"personType":ptype,"isFaved":isFaved}
		myPagination("courses", data, "/resource/myResource");
	}
	...
	function myUserFav(obj) {
		...
		myPagination("user_fav_list", {}, "/resource/user/fav/list");
	}

	function changeMyResourceType(isFave,obj) {
		isFaved=isFave;
		...
		var data={"resType":"${resType}","personType":"${personType}"};
		if(isFave!=null){
			data.isFaved=isFave;
		}
		myPagination("courses", data, "/resource/myResource");
	...
	function search() {
		...
		var url = "/resource/myResource?personType="+ptype+"&isFaved="+isFaved;
	...
	function shareIndex(id){
		var mes = "共享";
		var url = "${pageContext.request.contextPath}/resource/shareIndex?edit?personType="+ptype+"&dm=${dm}&id="+id;
				...
				success: function(data) {
					if(data=="2") {
						...
						if("${personType}"!='res_school'){
							location.href = "${pageContext.request.contextPath}/resource/myResource?resType=${resType}&index=index&personType=res_person&dm=${dm}";
						}else{
							location.href = "${pageContext.request.contextPath}/resource/myResource?resType=${resType}&index=index&personType=res_school&dm=${dm}";
						}
					}
				}
			});
	...
	function verifyDelete(id,personType) {
		...
		$.confirm("确定取消共享此资源吗？", function() {
			$.ajax({
				url: "${pageContext.request.contextPath}/resource/verifyDelete",
				type: "POST",
				data: {
					"Id":id,
	...
	function removeFav(id) {
		$.ajax({
			...
			success: function() {
				$.success("取消收藏成功！");
				location.href = "${pageContext.request.contextPath}/resource/myResource?index=index&resType=${resType}&personType=fav_resource&dm=${dm}";
			}
		});
	}
