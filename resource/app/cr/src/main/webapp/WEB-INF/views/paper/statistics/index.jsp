...
<head>
    ...
    <link href="${pageContext.request.contextPath}/res/qyjx/css/statistics/dxa_statistics.css" rel="stylesheet">
    <script src="${pageContext.request.contextPath}/res/plugin/Highcharts-4.0.3/js/highcharts.js"></script>
    <script src="${pageContext.request.contextPath}/res/js/common/jquery.nicescroll.js"></script>
...
<div class="a3-3" style="overflow:hidden">
    <div class="a3-3-top">
        <span class="fl banji">试卷统计 >> ${paPaper.title} >> ${teamName}</span>
        <span class="fr anniu">
            <a href="javascript:" class="btn-blue mgr10" onclick="exportExcel()">导出数据</a>
            <a href="javascript:" class="btn-lightGray">返回</a>
        </span>
    </div>
    <!-- 描点 -->
    <div class="sz_miao">
...
<div class="more_div" style="display:none;">
    ...
    <div class="a3-3-top">
        ...
        <span class="fr anniu">
            <a href="javascript:" class="btn-lightGray" onclick="back_current()">返回</a>
        </span>
    </div>
    ...
    <div class=" stxq" style="display:none">
        <iframe src="" allowFullScreen marginheight="0" marginwidth="0" frameborder="0" scrolling="yes"
...
<script type="text/javascript">
    ...
    function exportExcel() {
        ...
        //window.location.href = "${pageContext.request.contextPath}/statistics/lp/export?examId=" + examId + "&unitIds=" + unitIdStr;
    }
    ...
    function getData() {
        ...
        $.ajax({
            url: "${pageContext.request.contextPath}/statistics/pa/tj/getData",
            type: 'GET',
            async: false,
            data: {
                "taskId": ${taskId},
    ...
    $(function () {
        ...
        $('body').on('click', '.select_mk .m_select a', function () {
            if ($(this).hasClass('all')) {
                ...
                if ($('.select_mk .m_select a.on').length != 1 && $('.select_mk .m_select a').length != 2) {
                    $(this).toggleClass('on');
                    if ($(this).hasClass('on')) {
                        ...
                        $('.select_mk .m_select a.all').removeClass('on')
                    }
                } else {
                    if (!$(this).hasClass('on')) {
                        $(this).addClass('on');
    ...
    function check_wcqk(obj) {
        $(".more_div .banji").text("试卷统计 >> ${paPaper.title} >> ${teamName}");
        $('.more_div .bjwcl_pmb').hide();
        ...
        if (status === "1") {
            html += "<tr><th>" + $(obj).children('p').eq(0).text() + "</th><th>完成</th><th>" + time + "</th></tr>";
        } else {
            html += "<tr><th>" + $(obj).children('p').eq(0).text() + "</th><th>未完成</th><th></th></tr>";
        }
                ...
                if (data === 1) {
                    layer.open({
                        ...
                        cancel: function () {
                            ...
                            window.location.href = "${pageContext.request.contextPath}/statistics/lp/tj/index?examId=" + examId + "&number=" + number;
                        },
                    });
                    var examId =${param.examId};
                    var number = $('.a3-3-top ul li.select1').index() + 1;
                    setTimeout('window.location.href="${pageContext.request.contextPath}/statistics/lp/tj/index?examId=' + examId + '&number=' + number + '"', 2000);
                }
            }
        });
        return flag;
