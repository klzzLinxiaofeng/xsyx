            ...
            <dl>
                <dt>
                    <c:choose>
                        ...
                        <c:otherwise>
                            <a href="${pageContext.request.contextPath}/resource/Play?id=${res.id}">
                                <img attai="${res.iconType }" src="${thumbFn:getConvertedUrl('',res.iconType, pageContext.request.contextPath, pageContext.request.serverName)}">
                            </a>
                        </c:otherwise>
                    </c:choose>
		                    	...
		                    	<span class="${iconFn:getClassName(res.iconType)} icon-file res-iconb"></span>
                           		<span style="overflow:hidden;width:338px;float:left;display:block; white-space:nowrap; text-overflow:ellipsis; word-wrap: normal;">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <a href="${pageContext.request.contextPath}/resource/Play?id=${res.id}" title="${res.title}">${res.title}</a>
                                        </c:otherwise>
                                    </c:choose>
...
<script type="text/javascript">
    ...
    function download2(resId){
        location.href="${pageContext.request.contextPath}/resource/download?Id=" + resId;
...
function editMicro(id) {
    var mes = "编辑";
    var url = "${pageContext.request.contextPath}/resource/edit?personType=${personType}&dm=${dm}&id="+id;
    $.initWinOnTopFromLeft(mes, url, '1000', '800');
}
</script>