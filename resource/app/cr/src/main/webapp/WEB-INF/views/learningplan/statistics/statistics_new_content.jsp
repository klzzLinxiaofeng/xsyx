        ...
        <div class="swiper-wrapper" style="display:block;">
            <div class=" swiper_1" style="width:100%;">
                <div class="first_d">
                    <div class="pm_div">
                        <ul>
                            <li class="l1"><a href="javascript:void(0)"><p class="p2">年级排名</p>
                                <p class="p1">${rank}</p></a></li>
                            <li class="l2"><a href="javascript:void(0)"><p class="p2">年级完成率</p>
                                <p class="p1">${gradeFinishRate}%</p></a></li>
                            <li class="l3"><a href="javascript:void(0)"><p class="p2">本班完成率</p>
                                <p class="p1">${finishRateSelf}%</p></a></li>
                            <li class="l4"><a href="javascript:void(0)"><p class="p2">完成人数</p>
                    ...
                    <div class="first_one">
                        <div class="bjwcl_pmb">
                            ...
                            <div class="table_1" id="table_1">
                                <table class="table">
                                    ...
                                    <tbody>
                                    <c:set var="rank_list" value="1"></c:set>
                                    <c:forEach items="${teamList}" var="item" varStatus="xh">
                                        <c:if test="${xh.index==0}">
                                            <tr class="">
                                                <td>1</td>
                                                <td>${item.teamName}</td>
                                                <td class="f_red">${item.finishRate}%</td>
                                                <td>${item.peopleCount}/${item.allPeopleCount}</td>
                                                <td>${item.teamAvgScore}</td>
                                            </tr>
                                        </c:if>
                                        <c:if test="${xh.index!=0}">
                                            <c:if test="${teamList[xh.index].finishRate!=teamList[xh.index-1].finishRate}">
                                                <c:set var="rank_list" value="${rank_list+1}"></c:set>
                                            </c:if>
                                            <tr class="">
                                                <td><c:out value="${rank_list}"/></td>
                                                <td>${item.teamName}</td>
                                                <td class="f_red">${item.finishRate}%</td>
                                                <td>${item.peopleCount}/${item.allPeopleCount}</td>
                                                <td>${item.teamAvgScore}</td>
                        ...
                        <div class="bjwcl_pmb xswcjd">
                            <div class="title">学生完成进度
                                <div class="wcrs fr">
                                    <p class="p1">
                                        已完成:<span class="ywc">${peopleCount}</span>/<span
                                            class="">${fn:length(studentList)}</span>人</p>
                                </div>
                            </div>
                            ...
                            <div id="first_container_1" class="first_container_1">
                                <ul>
                                    <c:forEach items="${studentList}" var="item">
                                        <li onclick="check_wcqk(this)" data-status="${item.finishStatusDetail}"
                                            data-time="${item.finishTimeDetail}">
                                            <p class="p_1">${item.studentName}</p>
                                            <c:choose>
                                                <c:when test="${item.personRateStr=='100'}">
                                                    <div class="div_2">
                                                        <p class="p_2 all_ok_jdt"
                                                           style="width:${item.personRateStr}%"></p>
                                                    </div>
                                                    <p class="p_3 all_ok_wz">${item.personRateStr}%</p>
                                                </c:when>
                                                <c:when test="${item.personRateStr=='0'}">
                                                    <div class="div_2">
                                                        <p class="p_2 no_ok_jdt"
                                                           style="width:${item.personRateStr}%"></p>
                                                    </div>
                                                    <p class="p_3 no_ok_wz">${item.personRateStr}%</p>
                                                </c:when>
                                                <c:otherwise>
                                                    <div class="div_2">
                                                        <p class="p_2 part_ok_jdt"
                                                           style="width:${item.personRateStr}%"></p>
                                                    </div>
                                                    <p class="p_3 part_ok_wz">${item.personRateStr}%</p>
                                                </c:otherwise>
                                            </c:choose>
                                        </li>
                                    </c:forEach>
...
<div class="mk answer-statistics">
    ...
    <div class="mk_title">
        ...
        <p class="p2">*已选择：
            <span>
                <c:forEach items="${unitNames}" var="item" varStatus="xh">
                    ${item}
                    <c:if test="${xh.last==false}">
                        、
                    </c:if>
                </c:forEach>
            </span>
    ...
    <div class="pm_div">
        <ul>
            <li class="l5"><a href="javascript:void(0)"><p class="p2">年级平均分</p>
                <p class="p1">${gradeAvgScore}</p></a></li>
            <li class="l6"><a href="javascript:void(0)"><p class="p2">班级平均分</p>
                <p class="p1">${teamAvgScore}</p></a></li>
            <li class="l7"><a href="javascript:void(0)"><p class="p2">已提交人数</p>
                <p class="p1">${submitPeoples}</p></a></li>
            <li class="l8"><a href="javascript:void(0)"><p class="p2">年级排名</p>
                <p class="p1">${rankScore}</p></a></li>
        </ul>
        <div class="clear"></div>
    </div>
    <div class="bjwcl_pmb xsdtck">
        <div class="title">学生答题情况
            <div class="wcrs fr">
                <p class="p1">
                    已完成:<span class="ywc">${submitPeoples}</span>/<span class="">${fn:length(answerList)}</span>人</p>
            </div>
        </div>
        <input type="hidden" id="paperUnitIds" value="${paperUnitIds}">
        <div class="second_container_1">
            <ul>
                <c:forEach items="${answerList}" var="item" varStatus="xh">
                    <c:choose>
    ...
    <div class="bjwcl_pmb xswcjd">
        ...
        <div class="second_container_2">
            ...
            <%--<div class="title_center">【<span class="color-2ba0f7">预习自测</span>】1-3题得分率对比图</div>--%>
            <%--<div id="container_1" class='container' style='width:876px;margin: 0 auto'></div>--%>
            <%--</div>--%>
            <%--<div class="container_list">--%>
            <%--<div class="title_center">【<span class="color-2ba0f7">课堂检测</span>】1-3题得分率对比图</div>--%>
            <%--<div id="container_2" class='container' style='width:876px;margin: 0 auto'></div>--%>
            <%--</div>--%>
        </div>
    </div>
</div>
...
<div class="mk single-analysis" style="margin-bottom:20px">
    ...
    <c:forEach items="${singleList}" var="single" varStatus="xh1">
        <div class="container_list bjwcl_pmb">
            <div class="title_center">【<span class="color-2ba0f7">${unitNames[xh1.index]}</span>】单题分析</div>
            <table class="table">
                <thead>
                <tr>
                    <th>题号</th>
...
<script>
        ...
        if (question_list !== '') {
            ...
            $.each(question_list, function (index, n) {
                ...
                for (var i = 0; i < k; i++) {
                    ...
                    if (i === k - 1) {
                        $('<div class="container_list"><div class="title_center">【<span class="color-2ba0f7">' + h + '</span>】' + (k1 + 1) + '-' + k3 + '题得分率对比图</div><div id="container_' + index + i + '" class="container" style="width:876px;margin: 0 auto"></div></div>').appendTo(".second_container_2")
                    } else {
                        $('<div class="container_list"><div class="title_center">【<span class="color-2ba0f7">' + h + '</span>】' + (k1 + 1) + '-' + k2 + '题得分率对比图</div><div id="container_' + index + i + '" class="container" style="width:876px;margin: 0 auto"></div></div>').appendTo(".second_container_2")
                    }
    ...
    let gradeContrast = function (examId, questionUuid, unitId, obj) {
        $.getJSON("${pageContext.request.contextPath}/statistics/new/tj/questionRate?examId=" + examId + "&questionUuid=" + questionUuid + "&unitId=" + unitId, function (data) {
