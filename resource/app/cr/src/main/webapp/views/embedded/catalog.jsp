...
<script type="text/javascript">
	...
	function textBookCatalogListByCondition(stageCode, subjectCode, gradeCode,
			publisherId, volumn, functionName, div, prefixId,personType) {
		var isPublish=1;
		if(personType=='res_school'){
			var isPublish=0;
		}
		textBookCatalogList(div, stageCode, gradeCode, subjectCode, volumn,
				publisherId, 'select', '', functionName, prefixId,isPublish);

	}

	function textBookCatalogList(div, stageCode, gradeCode, subjectCode,
			volumn, publisherId, type, catalogId, functionName, prefixId,isPublish) {
		...
		if ('select' == type) {
	     ...
	     var URL="${pageContext.request.contextPath}/teach/textBookMaster/master/textBookCatalogList";
	     if(isPublish==0){
            URL="${pageContext.request.contextPath}/teach/textBookMaster/master/resTextBookCatalogList";
	     }
		$.ajax({    type : "post",
					url : URL, 
					data : {
