...
(function($){
	$.initAccordionNav = function(contextPath) {
		 /*
	    ...
	    $('.accordion-nav li').each(function () {
	        if ($(this).children('ul').length > 0) {
	            if ($(this).children('ul').is(":visible")) {
	                $(this).prepend('<img src="' + contextPath + '/res/plugin/falgun/images/imgOnOpen.png" />');
	            }
	            else {
	                $(this).prepend('<img src="' + contextPath + '/res/plugin/falgun/images/imgOffClosed.png" />');
	            }
	        }
	    });
	    /*
	    ...
	    */
	    $('.accordion-nav img').click(function() {
	        if ($(this).parent('li').children('ul').html() != null) {
	            ...
	            $(this).parent('li').parent('ul').children('li').children('img').attr('src', contextPath + '/res/plugin/falgun/images/imgOffClosed.png');
	            $(this).delay(100).is(':hidden');
	            if ($(this).parent('li').children('ul').css('display') == "block") {
	                $(this).parent('li').children('ul').hide(slideSpeed);
	                $(this).attr('src', contextPath + '/res/plugin/falgun/images/imgOffClosed.png');
	            } else {
	                $(this).parent('li').children('ul').show(slideSpeed);
	                $(this).attr('src', contextPath + '/res/plugin/falgun/images/imgOnOpen.png');
	            }
	            return false;
	        }
