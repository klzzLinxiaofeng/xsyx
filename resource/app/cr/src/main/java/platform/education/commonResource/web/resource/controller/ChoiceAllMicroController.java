...
@Controller
@RequestMapping(value = "/choiceAll")
public class ChoiceAllMicroController {
	@Autowired
    @Qualifier("storage")
	...
	public String process() {
	    ...
	    for(ResourceLibrary rl : list){
			 ...
			 * 初中语文科目
			 */
			System.out.println("微课资源 初中语文科目生成中");
			 ...
			 * 小学数学科目
			 */
			System.out.println("微课资源 小学数学科目生成中");
			 ...
			 * 初中数学科目
			 */
			System.out.println("微课资源 初中数学科目生成中");
			...
			System.out.println("微课资源 初中数学科目生成结束");
			/**
			 * 小学英语科目
			 */
			System.out.println("微课资源小学英语生成中");
			...
			System.out.println("微课资源小学英语生成结束");
			/**
			 * 初中英语科目
			 */
			System.out.println("微课资源初中英语生成中");
			...
			System.out.println("微课资源初中英语生成结束");
			/**
			 * 感兴趣的资源
			 */
	    ...
	    try {
			...
			this.resourceOperationService.updateAll();
	...
	@ResponseBody
	public String generateIndexDisplay(){

		synchronized (this) {
			boolean flag = false;
			
			int[] resTypes = ResourceDisplayCondition.resType;
			for (int i = 0; i < resTypes.length; i++) {
				int resType = resTypes[i];//null
				try {
					System.out.println("================资源类型为"+resType+"同步展示表开始=================");
					
					this.generateResourceDisplay(flag, resType);
					System.out.println("=================资源类型为"+resType+"同步展示表完成=================");
				} catch (Exception e) {
					System.out.println("=================资源类型为"+resType+"同步展示表完成=================");
					//e.printStackTrace();
				}
			...
			if(!flag){
				...
				//删除
				System.out.println("================资源旧数据删除开始=================");
				this.resourceDisplayService.deleteOld();
				System.out.println("================资源旧数据删除完成=================");
				//更新
				System.out.println("================资源新数据更新成旧数据开始=================");
				this.resourceDisplayService.updateNew();
				System.out.println("================资源新数据更新成旧数据结束=================");
			}
			return ResponseInfomation.OPERATION_SUC;
		}
	...
	/**
	 * 资源生成到展示表，各个学校访问展示表基础资源数据
	 * @param flag
	 * @param resType
	 * @throws Exception
	 */
	private void generateResourceDisplay(boolean flag,int resType) throws Exception{
		//int resType = 1;//null
		Order order = new Order();
		order.setProperty("create_date");
		order.setAscending(false);
		...
		ResourceDisplayCondition condition = new ResourceDisplayCondition();
		condition.setResType(resType);
		
		
		
		String[] subjectGradeCodeArray = ResourceDisplayCondition.subjectGradeCodeArray;
		
		for (int i = 0; i < subjectGradeCodeArray.length; i++) {
			String subjectGradeCode = subjectGradeCodeArray[i];
			String subjectCode = subjectGradeCode.split("\\+")[0];
			String gradeCode = subjectGradeCode.split("\\+")[1];
			String resourceTypeName = "";
			if(resType == 1){
				resourceTypeName = "微课资源";
			}else{
				resourceTypeName = "学科资源"+resType;
			}
			System.out.println("============"+resourceTypeName+"同步中，共"+subjectGradeCodeArray.length+"条,第"+(i+1)+"条>>>"+subjectGradeCode);
			condition.setSubjectCode(subjectCode);
			condition.setGradeCode(gradeCode);
			//condition.setVersionCode(ResourceDisplayCondition.versionCode);
			try {
				if(flag){//不强制生成
					List<ResourceDisplay> list = this.resourceDisplayService.findResourceDisplayByCondition(condition,page,order);
					if(list != null&&list.size()>0){
						
					}else{
						//调用生成方法
						this.resourceDisplayService.createResourceDisplayByResourceCatalog(condition);
						//List<ResourceDisplay> listTemp = this.resourceDisplayService.findResourceDisplayByCondition(condition,page,order);
