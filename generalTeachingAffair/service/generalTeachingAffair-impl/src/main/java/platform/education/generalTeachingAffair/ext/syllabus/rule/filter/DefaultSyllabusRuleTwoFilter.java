...
public class DefaultSyllabusRuleTwoFilter implements SyllabusRuleFilter<SyllabusVo>{
	...
	public ResponseDataCarrier doFilter(DataCarrier<SyllabusVo> dc) {
		...
		condition.setTeamId(syllabusVo.getTeamId());
		...
		//判断某个班级在同一个学期内同一天某一节课是否已经安排了课程，如果大于0说明已经安排
			...
			for(SyllabusVo voTmp : syllabusVos) {
				if(currentLessonId != null) {
					...
					if(!currentLessonId.equals(tempLessonId)) {
						responseData.setInfo(RuleResponseInfo.RULE_TWO);
