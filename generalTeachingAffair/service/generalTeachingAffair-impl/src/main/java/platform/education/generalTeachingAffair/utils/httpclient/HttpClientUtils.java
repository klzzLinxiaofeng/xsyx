package platform.education.generalTeachingAffair.utils.httpclient;
...
import platform.education.generalTeachingAffair.utils.httpclient.core.*;
...
//                                        .ssl()
    ...
    public static String get(String url){
        try {
            ...
            HttpRequestResult httpRequestResult = get(config);
            if(httpRequestResult.getCode()==200){
                return httpRequestResult.getResponseText();
            }else{
                logger.error("get请求：["+url+"]响应异常，相应信息：["+httpRequestResult+"]");
            }
        } catch (Exception e) {
            logger.error("get请求：["+url+"]发送失败",e);
        }
        return null;
    }
     ...
     * @param url
     * @param json
     * @return 响应体字符串，异常则返回null
     */
    public static String postJson(String url,String json){
        try {
            HttpRequestConfig postConfig = HttpRequestConfig.create().url(url)
                    .addHeader("Content-Type", "application/json")
                    .httpEntityType(HttpEntityType.ENTITY_STRING);
            postConfig.json(json);
            HttpRequestResult result = HttpClientUtils.post(postConfig);
            if(result.getCode()==200){
                return result.getResponseText();
            }else{
                logger.error("post请求：["+url+"]响应异常，相应信息：["+result+"]");
            }
        } catch (Exception e) {
            logger.error("post请求：["+url+"]发送失败");
            return null;
        }
        return null;
    }
