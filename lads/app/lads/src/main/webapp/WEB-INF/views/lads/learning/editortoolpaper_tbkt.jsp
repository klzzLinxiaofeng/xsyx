...
<!DOCTYPE html>
<h3 class="editor-title">${requestScope.title}</h3>
...
<script type="text/javascript">
    function initEditorContent(id) {
        ...
        if (uploadList != null && uploadList != "" && uploadList.length > 0 && content != null && content != "") {
            ...
            var html = "<a onclick=\"changeEditorTab('upload', '" + id + "');\">下载资料</a>"
                    + "<a onclick=\"changeEditorTab('edit', '" + id + "');\">阅读资料</a>";
    ...
    $(function() {
        ...
        if (!previewMode) {
            $.ajax({
                ...
                data: {"toolId": "${requestScope.id}", "editorScore": "${requestScope.editorScore}", "userId": "<lads:getEmbedUser sysType="${requestScope.sysType}"/>"},
                type: 'POST', //请求方式
                async: true,
                cache: false//是否缓存
            });
