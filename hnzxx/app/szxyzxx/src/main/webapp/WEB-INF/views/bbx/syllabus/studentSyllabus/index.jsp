					...
					<div class="widget-head">
						...
						<c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 1)}">
							<div class="s1 s1_bg" id="classMasterSearch">
								<input id="studentId" data-id="" type="text" style="width: 160px;" name="studentId" 
									value="" placeholder="学生姓名" />
								<div class="content">
									<div class="click">	
...
<script type="text/javascript">  
	...
	function selectedHandler(data) {
		...
		$("#studentId_select").val(names[0]);
		$("#studentId").attr("data-id", ids[0]);
	}
	
	function search() {
		var val = {};
		var id = "syllabus_list_content";
		var url = "/bbx/StudentSyllabus/index?sub=list&dm=${param.dm}";
	...
	$('#syllabus_list_content').on('click','td .new_pj',function(){
		if($(this).hasClass('evaluate')){
			...
			$.initWinOnTopFromLeft('添加评价', '${ctp}/bbx/syllabusEvaluate/creator?syllabusLessonId='+syllabusLessonId, '700', '550');
		}else{
			//考勤
			var syllabusLessonId = $(this).attr("data-kbdm");
// 			var xq = $(this).parent().parent().attr("data-xq");
			$.initWinOnTopFromLeft('查看考勤', '${ctp}/bbx/attendancesSyllabus/student/input?lessonId='+syllabusLessonId, '700', '450');
		}
	})
</script>
</html>