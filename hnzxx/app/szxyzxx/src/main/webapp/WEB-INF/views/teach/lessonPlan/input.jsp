				...
				<div class="widget-container" style="padding: 20px 0 0;">
					<form class="form-horizontal tan_form" id="lessonplan_form" action="javascript:void(0);">
							<div class="control-group">
								...
								<div class="controls">
									<select id="schoolYear" name="schoolYear" disabled="disabled" class="{required : true}">
										<c:if test="${lessonPlan.id == null || lessonPlan.id == ''}">
							...
							<div class="control-group">
								...
								<div class="controls">
								<input type="text" id="title" name="title" class="{required : true,maxlength:50} span13" placeholder="如果不填，默认是文件名" value="${lessonPlan.title}">
								</div>
							</div>
							<div class="control-group">
								<label class="control-label">
							...
							<div class="control-group">
								<label class="control-label">
									<span style="color: red">*</span>上传教案：
								</label>
								<div class="controls">
									<c:if test="${isCK != 'disable'}">
										<input type="hidden" id="uploader" name="uploader">
									</c:if>
									<input type="hidden" name="fileId" id="fileId" value="${lessonPlan.fileId}"/>
									<p id="fileName">
										${entity.entityFile.fileName}
									</p>
									<c:if test="${isCK == 'disable'}">
										<a target="_blank" class="btn btn-success cz_btn" href='<entity:getHttpUrl uuid="${lessonPlan.fileId}"/>'>点击下载文件</a>
									</c:if>
								</div>
							</div>
						<div class="form-actions tan_bottom">
							<input type="hidden" id="id" name="id" value="${lessonPlan.id}" />
...
<script type="text/javascript">
	...
	$(function() {
		 ...
		 getSubjectByGradeCode("${lessonPlan.gradeCode}");
	...
	function saveOrUpdate() {
		if (checker.form()) {
			...
			var url = "${ctp}/generalTeachingAffair/lessonplan/creator";
			if ("" != $id) {
				$requestData._method = "put";
				url = "${ctp}/generalTeachingAffair/lessonplan/" + $id;
			}
			if($("#fileId").val()==""){
				$.alert("请上传教案！");
				return;
	...
	function getGradeBySchoolYear(){
		...
		$.post(url,null,function(data){
			...
			$("#gradeCode").val("${lessonPlan.gradeCode}");
			$("#gradeCode").chosen();
		},'json')
	}
	...
	function getSubjectByGradeCode(gradeCode){
		...
		$.post(url,null,function(data){
			...
			$("#subjectCode").val("${lessonPlan.subjectCode}");
...
// 							fileTypeExts : "*.*", //默认*.*
							...
							onUploadSuccess : function(file, data, response) {
								...
								if($("#title").val()==""){
									$("#title").val(getFileName($jsonObj.realFileName));
								}
							},
