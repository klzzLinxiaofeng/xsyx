...
<span id="name"></span> 发展评价状况数据分析</span>
...
function search(){
	...
	if(teamId != null && "" != teamId){
		$.post("${pageContext.request.contextPath}/teach/normalEvaluation/inmyself", $requestData, function(data, status) {
			if("success" === status) {
				$(".data").html(data);
			}
			loader.close();
...
function searchmyself(studentName,studentId){
	...
	loader.show();
	if(teamId != null && "" != teamId){
		$.post("${pageContext.request.contextPath}/teach/normalEvaluation/myselfList", $requestData, function(data, status) {
			if("success" === status) {
				$(".data").html(data);
			}
			loader.close();
