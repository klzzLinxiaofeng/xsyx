...
<script type="text/javascript"
	src="${pageContext.request.contextPath}/res/js/login/jquery-1.7.2.min.js"></script>
<style type="text/css">
canvas {
	/* border: 1px solid grey; */
	width: 786px;
	/* background-image:url("${eurlFn:convertUrl(microList[flag].bgpicturePath)}"); 
   background-image:url("${pageContext.request.contextPath}/res/images/record_bg_green.png");*/
...
.qp, .sx {
	background:
		url("${pageContext.request.contextPath}/res/css/extra/images/fullscrean.png")
...
.video_head {
	background:
		url("${pageContext.request.contextPath}/res/css/extra/images/weike_head_bg.png")
		no-repeat;
	position: absolute;
	top: 45px;
	left: 0;
...
.video_btn {
	background:
		url("${pageContext.request.contextPath}/res/css/extra/images/video_btn.png")
		no-repeat 50% 40%;
	width: 636px;
}
...
<body>
	<div class="canvas_play">
		<div class="video_head">
			<div class="video_btn">
				...
				<div class="left">
					<img src="${eurlFn:convertUrl(microList[0].logoPath)}">
		...
		<canvas id="canvas" onclick="is_palying()" class="canvas">您的浏览器不支持 canvas 标签。</canvas>
		<canvas id="canvas_images">您的浏览器不支持 canvas 标签。</canvas>
		<div class="audio_play">
			<audio id="audio" controls="controls" onplay="mediaPlay()"
				onpause="mediaPause()" onseeking="mediaSeeking()"
				onseeked="mediaSeeked()" onended="mediaEnded()">
				<source type="audio/mpeg"
					src="${eurlFn:convertUrl(microList[flag].mediaPath)}" />
				您的浏览器不支持 audio 标签。
			</audio>
			<a href="javascript:void(0)" onclick="qp(this)" class="qp"></a> <a
				href="javascript:void(0)" onclick="sx(this)" class="sx"
				style="display: none"></a>
		</div>
		<%-- <img src="${eurlFn:convertUrl(microList[flag].bgpicturePath)}" hidden id="img"> --%>
    ...
    function mediaEnded() {
      ...
      var data = {};
      data.microId = $("#currentMicroId").val();
...
//     }
    ...
    function writer() {
		...
		function sx(obj) {
			...
			precent = 636 / h5_width;
			canvasWidth = 636;
			canvasHeight = h5_height * precent;
			canvas.width = 636;
			canvas.height = h5_height * precent;
			canvas_images.width = 636;
		...
		$(function() {
			...
			var propertyPath = $("#propertyPath").val();
			$("canvas").css("width", "636px");
