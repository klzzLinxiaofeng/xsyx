							...
							<c:forEach items="${studentVoList}" var="studentVo">
								<c:choose>
									...
									<a href="javascript:void(0);" data-student-id="${studentVo.userId}" class="on">
										<span class="left stu-head"><%-- <img src="${studentVo.entityId}"/> --%>
											<c:choose>
												<c:when test="${empty studentVo.entityId}">
													<img src="${ctp}/res/images/no_pic.jpg"/> 
												</c:when>
												<c:otherwise>
													<img src="${studentVo.entityId}"/> 
												</c:otherwise>
											</c:choose>
										</span>
...
<script type="text/javascript">
	...
	function saveOrUpdate() {
		...
		var url = "${pageContext.request.contextPath}/bbx/bpTeamDutyUser/saveDutyUser";
