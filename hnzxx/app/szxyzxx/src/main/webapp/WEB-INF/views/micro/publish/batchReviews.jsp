			...
			<div style="margin-bottom: 0" class="content-widgets">
				<div style="padding: 20px 0 0;" class="widget-container">
							...
							<ul>
								<c:if test="${allList!=null&&fn:length(allList)>0}">
									<li id="allDivLi" onclick="chooseDiv('allDiv')" class="active">
										<a href="javascript:void(0)"><input id="allDivC" checked
											onclick="checkStudent('allDiv')" type="checkbox">全部</a>
									</li>
								</c:if>
								<c:if test="${finishedList!=null&&fn:length(finishedList)>0}">
									<li id="finishedDivLi" onclick="chooseDiv('finishedDiv')">
										<a href="javascript:void(0)"><input id="finishedDivC"
											onclick="checkStudent('finishedDiv')" type="checkbox">已完成</a>
									</li>
								</c:if>
								<c:if
									test="${partFinishedList!=null&&fn:length(partFinishedList)>0}">
									<li id="partFinishedDivLi"
										onclick="chooseDiv('partFinishedDiv')"><a
										href="javascript:void(0)"><input id="partFinishedDivC"
											onclick="checkStudent('partFinishedDiv')" type="checkbox">部分完成</a>
									</li>
								</c:if>
								<c:if
									test="${unFinishedList!=null&&fn:length(unFinishedList)>0}">
									<li id="unFinishedDivLi" onclick="chooseDiv('unFinishedDiv')">
										<a href="javascript:void(0)"><input id="unFinishedDivC"
											onclick="checkStudent('unFinishedDiv')" type="checkbox">未做</a>
									</li>
								</c:if>
							</ul>
						</div>
							...
							<div id="allDiv" class="tx">
								<ul>
									...
									<c:forEach items="${allList}" var="st">
										<li><img
											src="<avatar:avatar userId='${st.id}'></avatar:avatar>">
											<input value="${st.userId}" type="checkbox">
											<p class="name">${st.name}</p></li>
									</c:forEach>
								</ul>
							</div>
					...
					<div class="py">
						<p class="p1">请输入评语</p>
						<div class="p2">
							使用模板 <select id="template" onchange="setReviews()">
								<option>请选择</option>
								<option>完成的不错</option>
								<option>继续努力</option>
							</select>
					...
					<div class="form-actions tan_bottom">
						<button onclick="sendReviews();" class="btn btn-warning"
							onclick="javascript:void(0)" type="button">发送</button>
...
<script type="text/javascript">
	...
	function sendReviews() {
		...
		$
				.ajax({
					url : "${pageContext.request.contextPath}/micropublish/saveReviews",
					type : "POST",
					data : {
					...
					success : function() {
						...
						if (parent.core_iframe != null) {
							parent.core_iframe.window.location.href = "${pageContext.request.contextPath}/micropublish/publishDetails?index=index&microPublishedId=${microPublishId}&relateId=${relateId}&relateType=${relateType}";
						} else {
							parent.window.location.href = "${pageContext.request.contextPath}/micropublish/publishDetails?index=index&microPublishedId=${microPublishId}&relateId=${relateId}&relateType=${relateType}";
						}
						$.closeWindow();
						loader.close();
					}
