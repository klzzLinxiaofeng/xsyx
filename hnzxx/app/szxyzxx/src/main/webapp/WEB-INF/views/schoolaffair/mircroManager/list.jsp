...
<c:forEach items="${items}" var="item" varStatus="i">
    <tr id="${item.id}_tr">
        ...
        <td>${item.title}</td>
        <td>
            <c:if test="${item.coverUrl != null && item.coverUrl != ''}">
                <img src="${item.coverUrl}" style="width: 233px; height: 130px;">
            </c:if>
        </td>
        <td>${item.typeName}</td>
        ...
        <td class="caozuo">
            <c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 1)}">
                <button class="btn btn-green" type="button" onclick="loadCommentsPage('${item.id}')">评论管理</button>
            </c:if>
