...
.left{
    min-height: 433px;background: #fff;border:1px #d9dfe7 solid;margin: 0px 320px 0 15px;display: block;
}
.left span{
    text-align: center;
}
...
.left ul li a{
    background:#ccc;line-height: 30px;width: 9.6%;font-size: 12px;color: #fff;display: inline-block;margin-left: 2%;height: 30px;overflow: hidden;
}
.left ul li a{
    *display:inline;
}
...
.item{
    height: 40px;color: #fff;font-size: 12px;text-align: center;display: block;text-overflow: ellipsis;overflow: hidden;white-space: nowrap;
}
.item p{
/* 	width:77px; */
}
...
.right ul li{
    margin: 10px 0 0 10px;padding-top: 5px;width:80px;height:45px;
}
.right .item{
	float:left;
}
...
.item img{
    border-radius:50%;width: 30px;margin-top: 4px;
}
.item p{
      margin: 0 8px 0 0;color: #fff;float: right;
}
#student li img{
	width:35%;margin:4px 5px 0 6px;float:left;
}
#student li p{
	margin:8px 0 0 0;float:left;
}
#seat li a li img{
    width:20px;float: left;margin: 8px 0 0 5px;
}
#seat li a li p{
	float: left;margin: 1px 0 0 0;display: block;white-space: nowrap;overflow: hidden;text-overflow: ellipsis;width: 55px;
}
</style>
<body>
    ...
    <div class="choice">
        <ul>
            <li id="bj" class="bj">
            	<c:forEach items="${teamTeacherList}" var="teamTeacher" varStatus="i">
            		<a data-obj-id='${teamTeacher.teamId}' href="javascript:void(0)";'>${teamTeacher.teamName}</a>
            	</c:forEach>
            </li>
            <li id="js" class="js"></li>
            
        </ul>
        <button class="add" onclick="setClassRoom()" ><i class="fa fa-plus"></i>分配</button>
    </div>
    <div class="install">
        <button  class="seat" onclick="setSeatChartSize()"><i class="fa fa-cog"></i>设置座位</button>
        <div style="float:right;width:450px;">
        <button class="h" onclick="q()"><i class="fa fa-trash-o"></i>清空</button>
        <button class="b" onclick="save()"><i class="fa fa-save"></i>保存</button>
        <button class="zidong" onclick="zd()"><i class="fa fa-user"></i>自动分配</button>
        </div>
        <p id="className"></p>
    </div>
    <div class="position">
    <div class="you">
            <span><p>拖动到相应座位</p><p class="surplus">剩余<strong id="number" class="number">0</strong>人</p></span>
            <div class="right">
            	<ul id="student">
            	
<!--             	<div class="male item">张三<p>(1212340105)</p></div> -->
<!--                 <div class="female item">张萍萍<p>(1212340105)</p></div>  -->
				</ul>
            </div>
        </div>
        <div class="left">
            ...
            <ul id="seat">
<!--                 <li><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#"class="drop"></a><a href="#"class="drop"></a></li> -->
<!--                 <li><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#"class="drop"></a><a href="#"class="drop"></a></li> -->
<!--                 <li><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#"class="drop"></a><a href="#"class="drop"></a></li> -->
<!--                 <li><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#"class="drop"></a><a href="#"class="drop"></a></li> -->
<!--                 <li><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#" class="drop"></a><a href="#"class="drop"></a><a href="#"class="drop"></a></li> -->
...
<script type="text/javascript">
	...
	function ajaxClassFunction(classId){
		...
		var aj = $.ajax({
		    ...
		    success:function(data) {
		    	loadClassRoom(data);
		    	//loadStudentDoNotHasSeat();//重用下面的代码
		     },    
		     error : function() {
		         $.alert("异常！");    
		     }    
	...
	//选择完班级出现学生
	/* function ajaxGetStudent(teamId){
		var url = "${pageContext.request.contextPath}/teach/seatChart/getAjaxStudentList";
		var aj = $.ajax({
		    url: url,
		    data:'teamId=' + teamId,    
		    type:'post',    
			...
			if("success" === status) {
				...
				$.each(data,function(index,value){
					...
					if(value.studentSex=="1"||value.studentSex=="9"||value.studentSex===null){
						...
						$(".left .li:eq("+x+") a:eq("+y+")").append("<li data-obj-id=" + value.studentId + " data-obj-sex=" + value.studentSex + " data-obj-name=" + value.studentName + " class='male item'><img src="+ value.userIcon +"><p>"+value.studentName+"</p></li>");
					...
					if(value.studentSex=="2"){
						...
						$(".left .li:eq("+x+") a:eq("+y+")").append("<li data-obj-id=" + value.studentId + " data-obj-sex=" + value.studentSex + " data-obj-name=" + value.studentName + " class='female item'><img src="+ value.userIcon +"><p>"+value.studentName+"</p></li>");
						$(".left .li:eq("+x+") a:eq("+y+") li").addClass('assigned');
					}
					easyDraggable();
				});
	...
	//加载未分配座位的学生
	function loadStudentDoNotHasSeat(){
		$("#student").html("");
		var seatId = $("#seatId").val();
		var teamId = $("#classId").val();
		if(seatId!=''&&teamId!=''){
			var url = "${pageContext.request.contextPath}/teach/seatChart/loadStudentDoNotHasSeat";
			$.post(url, {"seatId":seatId,"teamId":teamId}, function(data, status) {
				if("success" === status) {
					$("#number").html(eval(data).length);
					data = eval("(" + data + ")");
					setStudent(data);
				}else{
					$.error("操作失败");
	...
	function setStudent(data){
		if(data.length > 0) {
			$.each(data,function(index,value){
				if(value.sex=="1"||value.sex=="9"||value.sex==""){
					var studentNumber = null;
					if(value.studentNumber===null||value.studentNumber==""){
						studentNumber = "暂无学号";
					}else{
						studentNumber = value.studentNumber;
					}
					$("#student").append("<li data-obj-id=" + value.id + " data-obj-sex=" + value.sex + " data-obj-name=" + value.name + " class='male item'>"+value.name+"<p>("+studentNumber+")</p></li>")
				}
				if(value.sex=="2"){
					var studentNumber = null;
					if(value.studentNumber===null||value.studentNumber==""){
					...
					$("#student").append("<li data-obj-id=" + value.id + " data-obj-sex=" + value.sex + " data-obj-name=" + value.name + " class='female item'>"+value.name+"<p>("+studentNumber+")</p></li>")
				}
			});
			easyDraggable();
		}
	...
	function setSeatChartSize(){
		...
		$.confirm("座位表将会清空？确定执行此次操作？", function() {
			$.initWinOnTopFromLeft('分配座位', '${pageContext.request.contextPath}/teach/seatChart/setSeatSize?seatId='+seatId, '600', '300');
		});
	}
	...
	function setClassRoom(){
		...
		//var requestDate = JSON.stringify($requestData);
		$.initWinOnTopFromLeft('分配教室', '${pageContext.request.contextPath}/teach/seatChart/setClassRoom?schoolYear='+schoolYear+'&gradeId='+gradeId+'&teamId='+classId, '600', '400');
	}
	...
	function save(){
		...
		loader.show();
		var url = "${pageContext.request.contextPath}/teach/seatChart/saveSeatItem";
	...
	function removeButtonClass(){
		$(".seat").css("background-color","#a8a8a8");
		$(".seat").attr('disabled',"true");
		$(".h").css("background-color","#a8a8a8");
		$(".h").attr('disabled',"true");
		$(".b").css("background-color","#a8a8a8");
		$(".b").attr('disabled',"true");
		$(".zidong").css("background-color","#a8a8a8");
		$(".zidong").attr('disabled',"true");
		$("#className").html("暂时未分配教室(请分配教室)");
	}
	//还原按钮打样式
	function addButtonClass(){
		$(".add").css("background-color","#47b9f4");
		$(".add").removeAttr('disabled');
		$(".seat").css("background-color","#5cb85c");
		$(".seat").removeAttr('disabled');
		$(".h").css("background-color",'red');
		$(".h").removeAttr('disabled');
		$(".b").css("background-color","#f0ad4e");
		$(".b").removeAttr('disabled');
		$(".zidong").css("background-color","#428bca");
		$(".zidong").removeAttr('disabled');
	}
</script>
</html>