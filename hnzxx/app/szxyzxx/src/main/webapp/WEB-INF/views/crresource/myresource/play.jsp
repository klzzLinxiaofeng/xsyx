                            ...
                            <div class="v_div">
                                <div class="fx_left">
                                    <div class="fx_1">
                                        <c:choose>
                                            <c:when test="${faved}">
                                                <a id="sca" href="javascript:void(0)"class="a1" style="background: url('/res/images/ysc.png') no-repeat 10px center;" onclick="fav('${res.id}')">已收藏</a>
                                            </c:when>
                                            <c:otherwise>
                                                <a id="sca" href="javascript:void(0)"class="a1" style="background: url('/res/images/wsc.png') no-repeat 10px center;" onclick="fav('${res.id}')">收藏</a>
                                            </c:otherwise>
                                        </c:choose>

                                        <c:choose>
                                            <c:when test="${liked}">
                                                <a id="dza" href="javascript:void(0)" class="a3" style="background: url('/res/images/ydz.png') no-repeat 10px center;" onclick="like('${res.id}', this)" style="width:0;"></a>
                                            </c:when>
                                            <c:otherwise>
                                                <a id="dza" href="javascript:void(0)" class="a3" style="background: url('/res/images/wdz.png') no-repeat 10px center;" onclick="like('${res.id}', this)" style="width:0;"></a>
                                            </c:otherwise>
                                        </c:choose>
...
function fav(id) {
	...
	$.post("${pageContext.request.contextPath}/crresource/fav", requestData, function(data, status) {
		if("success" === status) {
            var msg="收藏";
            var img="/res/images/ysc.png";
            var name="已收藏";
            if(faved){
                msg="取消收藏";
                img="/res/images/wsc.png";
                name="收藏";
            }
			if ("faved" === data) {
				$.success("已收藏过");
...
function like(id, obj) {
    ...
    $.post("${pageContext.request.contextPath}/crresource/like", requestData, function (data, status) {
        if ("success" === status) {
            var msg="点赞";
            var img="/res/images/ydz.png";
            if(liked){
                msg="取消点赞";
                img="/res/images/wdz.png";
            }
            if ("faved" === data) {
                $.success("已赞过");
            } else if ("success" === data) {
...
function updateScoring(resId, score) {
	if(tab==0) {
		$.ajax({
	        url: "${pageContext.request.contextPath}/crresource/updateScore",
	        type: "POST",
