            ...
            <div class="widget-head">
                <h3>
                    ...
                    <p class="btn_link" style="float: right;">
                        ...
                        <a id="downLoadExcel"  class="a2" href="#" onclick="daochu();" class="a2" >学生成绩导出</a>
                    ...
                    <div class="select_b">
                        ...
                        <div class="clear">
                            <div class="toggle-button-wrapper">
                                    ...
                                    <c:if test="${juese=='success'}">
                                        <input type="checkbox" id="toggle-button" name="switch">
                                    <label for="toggle-button" class="button-label">
                                        <span  class="circle"></span>
                                        <span id="pingji" class="text on">评级</span>
                                        <span id="defen" class="text off">得分</span>
                                    </label>
                                    </c:if>
                                <c:if test="${juese!='success'}">
                                    <input type="checkbox" id="toggle-button" disabled name="switch">
                                    <label for="toggle-button" class="button-label" disabled>
                                        <span  class="circle"></span>
                                        <span id="pingji" disabled class="text on">评级</span>
                                        <span id="defen" disabled class="text off">得分</span>
                                    </label>
                                </c:if>
...
<script type="text/javascript">
    $(function () {
        initSelect();
        var strs='<thead><tr role="row"><th>序号</th><th>学号</th><th>姓名</th><th>得分</th></tr></thead><tbody id="publicClass_list_content"></tbody>';
    ...
    function search() {
        ...
        var km =$("#km").val();
        var kl =$("#kl").val();
        var xs=$("#ks").find('option:selected').text();
        if (xn != null && xn != "") {
            val.schoolYear = xn;
            if (xq != null && xq != "") {
                val.schoolTrem = xq;
                    ...
                    if (bj != null && bj != "") {
                        ...
                        if(kl!=null && kl !=""){
                            ...
                            if (km != null && km != "") {
                                ...
                                var url = "/scoreFenXi/findByscoreStudentScore";
                                $.post(url,val,function (d) {
                                    ...
                                    if(node.is(':hidden')){　　//如果node是隐藏的则显示node元素，否则隐藏
                                        ...
                                        for(var a=0;a<dd.length;a++){
                                            strs+="<tr><td>"+(parseInt(a)+1)+"</td><td>"+dd[a].empCode+"</td><td>"+dd[a].stuName+"</td><td>"+dd[a].zongfen+"</td>";
                                            strs+="</tr></tbody>";
                                        }

                                    }else{
                                        //评价
                                        strs='<thead><tr role="row"><th>序号</th><th>学号</th><th>姓名</th><th>评级</th></tr></thead><tbody id="publicClass_list_content" >';

                                        for(var a=0;a<dd.length;a++){
                                            strs+="<tr><td>"+(parseInt(a)+1)+"</td><td>"+dd[a].empCode+"</td><td>"+dd[a].stuName+"</td>";
                                            //A++=100、100>A+≥95、95>A≥85、85>B≥75、75>C≥60、60>D（按总分比）
                                            if(dd[a].baifenbi==100){
                                                strs+="<td>A++</td>";
                                            }
                                            if(dd[a].baifenbi>=95 && dd[a].baifenbi<100){
                                                strs+="<td>A+</td>";
                                            }
                                            if(dd[a].baifenbi<95 && dd[a].baifenbi>=85){
                                                strs+="<td>A</td>";
                                            }
                                            if(dd[a].baifenbi<85 && dd[a].baifenbi>=75){
                                                strs+="<td>B</td>";
                                            }
                                            if(dd[a].baifenbi<75 && dd[a].baifenbi>=60){
                                                strs+="<td>C</td>";
                                            }
                                            if(dd[a].baifenbi<60){
                                                strs+="<td>D</td>";
    ...
    function daochu() {
        var url = "/scoreFenXi/daochuStudentFenXi";
        var nj = $("#nj").val();
        var bj = $("#bj").val();
        var xn = $("#xn").val();
        var kl =$("#kl").val();
    ...
    function initSelect(){
        ...
        $("#kl").change(function(){
            ...
            var bjId=$("#bj").val();
            if($(this).val()!="") {
                addOption("/scoreFenXi/findByExamName?schoolYear=" + $("#xn").val() + "&schoolTrem=" + $("#xq").val() + "&teamId=" + bjId+"&gradeId="+$("#nj").val()+"&examType="+$("#kl").val(), "ks", "id", "exam_name")
            }
        })
