...
<body>
	<div class="container-fluid">
		<jsp:include page="/views/embedded/navigation.jsp">
			...
			<jsp:param value="学术交流" name="title" />
					...
					<div class="content-widgets">
						<div class="widget-container">
							<div class="select_b">
								...
								<div class="select_div">
									<span>校内人员：</span>
									<input type="text" id="attendeesName" name="attendeesName" style="margin-bottom: 0; padding: 6px; width: 120px;" placeholder="关键字" value="">
								</div>
							...
							<table class="responsive table table-striped" id="data-table">
								...
								<tbody id="academicExchange_list_content">
									<jsp:include page="./list.jsp" />
								</tbody>
							</table>
							<jsp:include page="/views/embedded/jqpagination.jsp" flush="true">
								<jsp:param name="id" value="academicExchange_list_content" />
								<jsp:param name="url" value="/teach/academicExchange/index?sub=list&dm=${param.dm}" />
...
<script type="text/javascript">
	...
	function search() {
		var val = {
				"type" : $("#type").val(),
				"attendeesName" : $("#attendeesName").val()
		};
		var name = $("#name").val();
		if (name != null && name != "") {
			val.name = name;
		}
		var id = "academicExchange_list_content";
		var url = "/teach/academicExchange/index?sub=list&dm=${param.dm}";
		myPagination(id, val, url);
	}

	// 	加载创建对话框
	function loadCreatePage() {
		$.initWinOnTopFromLeft('创建', '${ctp}/teach/academicExchange/creator', '600', '500');
	}
//  加载编辑对话框
	function loadEditPage(id) {
		$.initWinOnTopFromLeft('编辑', '${ctp}/teach/academicExchange/editor?id=' +id, '600', '500');
	}
	
	function loadViewerPage(id) {
		$.initWinOnTopFromLeft('详情', '${ctp}/teach/academicExchange/editor?id=' + id + "&isCK=disable", '600', '500');
	}
	...
	function executeDel(obj, id) {
		$.post("${ctp}/teach/academicExchange/" + id, {"_method" : "delete"}, function(data, status) {
			if ("success" === status) {
				if ("success" === data) {
					$.success("删除成功");
					$("#" + id + "_tr").remove();
