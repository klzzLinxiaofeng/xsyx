...
//             nextLabel: 'Next',
    ...
    layui.use('upload', function(){
        ...
        var uploadInst = upload.render({
            elem: '#upFile' //绑定元素
            ,url: '${pageContext.request.contextPath}/Literacy/student/upLoadPingYuScore' //上传接口
        ...
        if (listSuccess.length == 0) {
            ...
            for (var i = 0, len = listSuccess.length; i < len; i++) {
                $("#successResultId").append("<tr id='data"+i+"'>" +
                    "<td>"+listSuccess[i]['school_year']+"</td>" +
                    "<td>"+listSuccess[i]['school_trem']+"</td>" +
                    "<td>"+listSuccess[i]['gradeName']+"</td>" +
                    "<td>"+listSuccess[i]['subjectName']+"</td>" +
                    "<td>"+listSuccess[i]['zhibiaoName']+"</td>" +
        ...
        if(listFail.length == 0){
            ...
            for(var k = 0, lenTemp = listFail.length; k < lenTemp; k++){
                $("#failResultId").append("<tr id='data2"+k+"'>" +
                    "<td>"+listFail[k]['school_year']+"</td>" +
                    "<td>"+listFail[k]['school_trem']+"</td>" +
                    "<td>"+listFail[k]['gradeName']+"</td>" +
                    "<td>"+listFail[k]['subjectName']+"</td>" +
                    "<td>"+listFail[k]['zhibiaoName']+"</td>" +
