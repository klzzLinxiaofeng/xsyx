...
<body>
	...
	<jsp:include page="/views/embedded/navigation.jsp">
			...
			<jsp:param value="所有考核查询" name="title" />
			...
			<div class="span12"  style="height: 43px;">
				<div class="content-widgets white" style="margin-bottom: 0;">
					<div class="widget-head">
						<h3>
							<span>所有考核查询-考核成绩列表</span>
							<p style="float:right;" class="btn_link">
								<a href="javascript:void(0)" class="a2" onclick="history.back();"><i class="fa fa-mail-reply"></i>返回</a>
					...
					<div class="xs_list">
						...
						<table class="table table-bordered responsive white " style="width:100%">
							...
							<tbody id="search_deltail_list">
								<jsp:include page="./all_search_detail_list.jsp"/>
							</tbody>
						</table>
						<div class="clear"></div>
						</div>
						<jsp:include page="/views/embedded/jqpagination.jsp" flush="true">
								...
								<jsp:param name="url" value="/schoolAffair/aptTask/allSearchDetail?sub=list&dm=${param.dm}" />
...
<%-- 					<jsp:param name="weeks" value="${weeks}" /> --%>
<%-- 					<jsp:param name="scopeType" value="${task.scopeType}" /> --%>
<%-- 					<jsp:param name="teachers" value="${teachers}" /> --%>
<%-- 					<jsp:param name="voList" value="${voList}" /> --%>
<%-- 				</jsp:forward> --%>
					<%
...
// 		alert(JSON.stringify(val));
		...
		var url = "/schoolAffair/aptTask/allSearchDetail?sub=list&dm=${param.dm}";
		myPagination(id, val, url);
	}
	function loadExportPage(){
		var taskId = "${task.id}";
		$.initWinOnTopFromLeft('导出', '${ctp}/schoolAffair/aptTask/loadExport?type=all&taskId=' + taskId, '850', '700');
	}
	$(function(){
