                    ...
                    <div class="xs_wk white">
                        <div class="xs_left">
                            ...
                            <div class="tea">
                                <div class="a1">
                                    <p class="p1">
                                        <span id="finishSpan_${micro.publishMicroLessonId}">0</span>
                                        /
                                        <span id="totalSpan_${micro.publishMicroLessonId}">${fn:length(micro.realMicroArray)}</span>
                                    </p>
                            ...
                            <%--<a href="${pageContext.request.contextPath}/learningDesignpublish/play?study=study&microPublishedId=${micro.publishMicroLessonId}&publisherId=${micro.publisherId}" class="start_study">开始学习</a>--%>
                        </div>
                        <div class="xs_right">
                            <ul>
    ...
    <script type="text/javascript">
        function getStatus() {
            $("div[id^='micrlList_']").each(function () {
                var ldId = $(this).find("input[id^='ldId_']").val();
                var microId = $(this).find("input[id^='microId_']").val();
                var microPublishId = $(this).find("input[id^='microPublishId_']").val();
                $.getJSON("<%=SysContants.COMMON_RESOURCE_BASE_PATH%>/lads/common/getUserFinishedStatus?ldId=" + ldId + "&userId=${sessionScope[sca:currentUserKey()].id}&callback=?",
                        function (data) {
                            if (data != null && data.status == 0) {
                                $("#ladsStatus_" + microPublishId).text("已完成");
                                var fc = parseInt($("#finishSpan_" + microPublishId).text());
                                var tc = parseInt($("#totalSpan_" + microPublishId).text());
                                if (fc < tc) {
                                    $("#finishSpan_" + microPublishId).text(fc + 1);
                                }
                                //提交记录
                                var data = {};
                                data.microId = microId;
                                data.finishedFlag = "<%=StudyFinishedFlag.FINISHED%>";
                                var publisherId = $("#ladsStatus_${microPublishId}").parent().parent().parent().find("input[type='hidden']").val();
                                ...
                                $("#ladsStatus_" + microPublishId).text("未完成");
                            }
                        });
            });
        }
        ...
        $(function () {
            ...
            var url = "${pageContext.request.contextPath}/ladspublish/ajaxGetStudyJson";
            $("div[id^='micrlList_']").each(function () {
