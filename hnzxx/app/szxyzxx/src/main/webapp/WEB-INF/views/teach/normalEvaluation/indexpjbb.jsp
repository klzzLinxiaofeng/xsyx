...
<!-- 							<a href="javascript:void(0)"  class="a3" onclick="appPjbb()"><i class="fa  fa-reply"></i>发展评价卡APP端评价报表</a> -->
							</p>
						</h3>
					</div>
						...
						<div class="widget-container">
							...
							<c:choose>
								<c:when test="${manager eq 'yes'}">
									<div class="select_div"><span>学期：</span> <select id="xq" style="width:150px;" onchange="week()"></select></div>
								...
								<c:otherwise>
									<div class="select_div"><span>学期：</span> <select id="xq" style="width:150px;" onchange="getClass()"></select></div>
...
// 	$requestData.week = week;
	loader.show();
	$.post("${pageContext.request.contextPath}/teach/normalEvaluation/bbxx", $requestData, function(data, status) {
		if("success" === status) {
			$("#kb_tb").html(data);
		}
		loader.close();
...
function myself(){
	...
	if(teamId != null && "" != teamId){
		$.post("${pageContext.request.contextPath}/teach/normalEvaluation/myself", $requestData, function(data, status) {
			if("success" === status) {
				$("#kb_tb").html(data);
			}
			loader.close();
