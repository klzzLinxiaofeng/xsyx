		...
		<div class="span12">
			<div class="content-widgets" style="margin-bottom: 0">
				<div class="widget-container" style="padding: 20px 0 0;">
					<form class="form-horizontal tan_form" id="SysBanner_form" action="javascript:void(0);">
							<div class="control-group">
								<label class="control-label">
									<span style="color: red;">*</span>标题：
								</label>
								<div class="controls">
								<input type="text" id="title" name="title" style="width: 250px;" class="span13" placeholder="标题" value="${SysBanner.title}">
								</div>
							</div>
							<div class="control-group">
								<label class="control-label">
								...
								<div class="controls">
									<input type="text" id="url" name="url" style="width: 250px;" placeholder="请填写完整的url，如:http://www.baidu.com" class="span13" value="${SysBanner.url}">
								</div>
							</div>
							<div class="control-group">
								<label class="control-label">
								...
								<div class="controls">
								<div>
									<input type="hidden" id="entityId" name="entityId" value="${SysBanner.entityId}">
								</div>
								<div id="zp" style="display:inline-block;">
											<div class="img_1">
												<c:if test="${(isCK==''||isCK==null) && (SysBanner.thumUrl !=null && SysBanner.thumUrl != '') }">
														<a data-id="${SysBanner.thumUrl}" onclick="reMove(this);">x</a>
												</c:if>
												<c:if test="${SysBanner.thumUrl !=null && SysBanner.thumUrl != ''}">
													<img src="${SysBanner.thumUrl}" onclick="Change(this);" style="width: 260px; height: 130px;">
												</c:if>
												<c:if test="${SysBanner.thumUrl ==null || SysBanner.thumUrl == ''}">
							...
							<div class="control-group">
								...
								<div class="controls">
<%-- 								<input type="text" id="pushState" name="pushState" class="span13" placeholder="" value="${SysBanner.pushState}"> --%>
								是<input type="radio" id="pushState" name="pushState" <c:if test="${SysBanner.pushState=='0'}">checked='checked'</c:if> placeholder="" value="0">
									&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
									否<input type="radio" id="pushState2" name="pushState"
										 <c:if test="${SysBanner.pushState == '1' || empty SysBanner.pushState}">checked='checked'</c:if>
									 placeholder="" value="1">
								</div>
							</div>
						<div class="form-actions tan_bottom">
							<input type="hidden" id="id" name="id" value="${SysBanner.id}" />
							<c:if test="${isCK==''||isCK==null}">
...
<script type="text/javascript">
	...
	function initValidator() {
		return $("#SysBanner_form").validate({
			errorClass : "myerror",
			rules : {
				"title" : {
					required : true,
	...
	function saveOrUpdate() {
		if (checker.form()) {
			...
			var $id = $("#id").val();
			var $requestData = formData2JSONObj("#SysBanner_form");
			var url = "${ctp}/sys/sysBanner/creator";
			if ("" != $id) {
				$requestData._method = "put";
				url = "${ctp}/sys/sysBanner/" + $id;
			}
			loader.show();
