...
//             nextLabel: 'Next',
    ...
    layui.use('upload', function(){
        ...
        var uploadInst = upload.render({
            elem: '#upFile' //绑定元素
            ,url: '${pageContext.request.contextPath}/examquestion/upLoadSubjectScore' //上传接口
            ,acceptMime:'application/vnd.ms-excel'
            ,exts:'xls'
            ,field:"fileUpload",
            data:{
                gradeId:${gradeId},
                examTeamSubjectId:${examId},
                teamId:${teamId},
                schoolYear:${xn},
                schoolTrem:"${xq}",
    ...
    function loadTable(listSuccess,listFail,biaotou){
        ...
        $("#success").append("<tr id='biaotou'><th>考试日程程序标识(不可修改)</th><th>学生程序标识(不可修改)</th><th>学号(不可修改)</th><th>姓名</th></tr>")
        $("#biaotou").append("<td>结果</td>");
        document.getElementById("error").innerHTML = "";
        $("#error").append("<tr id='biaotou2'><th>考试日程程序标识(不可修改)</th><th>学生程序标识(不可修改)</th><th>学号(不可修改)</th><th>姓名</th></tr>")
        ...
        if (listSuccess.length == 0) {
            ...
            for (var i = 0, len = listSuccess.length; i < len; i++) {
                $("#successResultId").append("<tr id='data"+i+"'><td>"+listSuccess[i]['id']+"</td><td>"+listSuccess[i]['studentId']+"</td><td>"+listSuccess[i]['empCode']+"</td><td>"+listSuccess[i]['stuName']+"</td></tr>");
        ...
        if(listFail.length == 0){
            ...
            for(var k = 0, lenTemp = listFail.length; k < lenTemp; k++){
                $("#failResultId").append("<tr id='dataq"+k+"'><td>"+listFail[k]['id']+"</td><td>"+listFail[k]['studentId']+"</td><td>"+listFail[k]['empCode']+"</td><td>"+listFail[k]['stuName']+"</td></tr>");
                $("#dataq"+k).append("<td>"+listFail[k]['error']+"</td>")
            }
        }
    }
    //退出
