...
<!DOCTYPE html>
    <c:forEach var="ms" items="${microList}">
        <li>
            <a href="${pageContext.request.contextPath}/micropublish/play?microId=${ms.resEnt.micro.id}&microPublishedId=${microPublishId}&publisherId=${param.publisherId}">
            ...
            <c:choose>
                ...
                <c:otherwise>
                    <c:choose>
                        <c:when test="${ms.resEnt.record.lastPlayTime!=null}">
                            <p class="de_1">上次播放到 ${ms.resEnt.formatLastPlayTime}</p>
                        </c:when>
                        <c:otherwise>
                            <p class="de_1">还没学习</p>
                        </c:otherwise>
            ...
            <p class="de_2">${ms.resEnt.micro.title}</p>
