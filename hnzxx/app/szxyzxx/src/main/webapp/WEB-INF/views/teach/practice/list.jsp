...
<c:forEach items="${items}" var="item" varStatus="i">
	<tr id="${item.id}_tr">
				...
				<td>${item.name}</td>
				<td><fmt:formatDate value="${item.practiceDate}" pattern="yyyy-MM-dd" /></td>
				<td>${item.position}</td>
				<td><p class="people_name" >${item.masterName}</p></td>
				<td><p class="people_name">${item.teachName},${item.studentName}</p></td>
		<td class="caozuo">
