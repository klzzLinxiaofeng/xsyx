...
<c:forEach items="${items}" var="item">
	<tr id="${item.id}_tr">
				...
				<td>
					<c:choose>
						<c:when test="${item.isOpen == true}">是</c:when>
						<c:otherwise>否</c:otherwise>
					</c:choose>
				</td>
				<%-- <c:if test="${item.isOpen == true}">是</c:if> --%>
		...
		<td class="caozuo">
			...
			<button class="btn btn-blue" type="button" onclick="loadEditPage('${item.id}', '${item.schoolId}', '${item.schoolName}');">编辑</button>
			</c:if>
