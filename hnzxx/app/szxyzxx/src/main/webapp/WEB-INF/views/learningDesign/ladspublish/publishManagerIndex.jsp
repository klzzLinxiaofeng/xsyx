                        ...
                        <div class="content-widgets">
                            <div class="widget-container">
                                    ...
                                    <c:forEach items="${classGradeMap}" var="cm">
                                        <c:forEach items="${cm.value}" var="teamMap">
                                            <c:forEach items="${teamMap}" var="team">
                                                <a onclick="changeClass('${team.key}');" href="javascript:void(0)" id="class_${team.key}" class="active" style="width:126px" data-type="${fn:split(team.value,'&&')[1]}">${fn:split(team.value,"&&")[0]}</a>
                                            </c:forEach>
                                        </c:forEach>
                                    </c:forEach>
                                </div>
                                ...
                                <jsp:include page="/views/embedded/jqpagination.jsp" flush="true">
                                    ...
                                    <jsp:param name="url" value="/ladspublish/publishManager" />
    ...
    <script type="text/javascript">
                                                    function changeClass(classId) {
                                                        ...
                                                        $(".kemu a").each(function() {
                                                            if (classId == $(this).attr("id").split("_")[1]) {
                                                        ...
                                                        var url = "/ladspublish/publishManager";
                                                    ...
                                                    function resetDate(mid, startDate, finishedDate) {
                                                        ...
                                                        $.initWinOnTopFromLeft(mes, '${pageContext.request.contextPath}/ladspublish/resetDate?publishId=' + mid + '&relateId=' + relateId + '&startDate=' + startDate + '&finishedDate=' + finishedDate, '700', '190');
                                                    }

                                                    function deletePublish(mid) {
                                                        $.confirm("确定删除这条已布置的课件吗?", function() {
                                                            $.ajax({
                                                                ...
                                                                success: function(html) {
                                                                    location.href = "${pageContext.request.contextPath}/ladspublish/publishManagerIndex?relateId=" + $(".kemu .active").attr("id").split("_")[1];
                                                                }
                                                            });
                                                        });
                                                    }
