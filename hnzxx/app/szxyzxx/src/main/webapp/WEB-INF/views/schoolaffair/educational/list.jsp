...
<c:forEach items="${items}" var="item" varStatus="i">
    <tr id="${item.id}_tr">
        ...
        <td>${item.name}</td>
        <td><img src="${item.picUrl}" style="width: 100px;"></td>
        <td>
            <c:if test="${item.object == 1} "> 教师</c:if>
            <c:if test="${item.object == 2}"> 学生/家长</c:if>
        </td>
        ...
        <td class="caozuo">
            <c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 1)}">
                <button class="btn btn-green" type="button" onclick="window.open('${item.url}')">查看统计</button>
            </c:if>
