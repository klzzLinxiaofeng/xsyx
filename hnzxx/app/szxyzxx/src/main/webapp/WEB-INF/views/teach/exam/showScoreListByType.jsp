...
<c:forEach items="${items}" var="item" varStatus="i">
	<tr class="tr_${item.id}">
		<td>
			${jcgcFn:getColValue("XY-JY-KSLX", item.examType, "name")}
		</td>
		<td>第 ${item.examRound} 轮</td>
...
<%-- 			<c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 2)}"> --%>
				<button class="btn btn-blue" type="button" onclick="examProModify('${item.examType}','${item.examRound}','${item.id}');">设置</button>
<%-- 			</c:if>  --%>
