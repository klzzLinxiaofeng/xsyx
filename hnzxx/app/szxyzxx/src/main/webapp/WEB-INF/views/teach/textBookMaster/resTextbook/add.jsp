						...
						<form class="form-horizontal tan_form" id="jctextbook_form" action="javascript:void(0);">
							...
							<div class="control-group">
								...
								<div class="controls">
									...
									onkeyup="checkFieldLength('name', '书名最大可输入50字');"  
                					onchange="checkFieldLength('name', '书名最大可输入50字', 100);">
							...
							<div class="control-group">
								...
								<div class="controls">
									<select  id="stageCode" name="stageCode"  onchange="getSubjectCode(this);" style="width:110px;"></select>
									<select id="subjectCode" name="subjectCode" onchange="getGradeCode(this);"  style="width:110px;">
										<option value="">请选择</option>
									</select>
									<select  id="gradeCode" name="gradeCode"  style="width:110px;">
										<option value="">请选择</option>
									</select>
...
<script type="text/javascript">
	...
	$(function() {
		...
		//学段
		$.stageCodeSelector("#stageCode",{"tn" : "jc_stage"}, '${jcTextbook.stageCode}', function(data) {
		...
		//学科
		/* $.resSelector("#subjectCode",{"tn" : "jc_subject", "pn" : "subjectClass", "value" : "1"}, '${jcTextbook.subjectCode}', function(data) {
			return {"val" : data.code, "title" : data.name};
		}, function() {
		...
		//年级
		/* $.resSelector("#gradeCode",{"tn" : "jc_grade"}, '${jcTextbook.gradeCode}', function(data) {
			return {"val" : data.code, "title" : data.name};
		}, function() {
		...
		//册次
		$.resSelector("#volumn",{"tn" : "jc_textbook_volumn"}, '${jcTextbook.volumn}', function(data) {
			return {"val" : data.code, "title" : data.name};
		}, function() {
	...
	function getSubjectCode(subjectCode) {
		var code = $(subjectCode).val();
		if(code!="") {
			$.ajax({  
				type:"get",  
				url:"${ctp}/teach/subject/list/json",  
				data:{"stageCode":code},
				success:function(data) {
					var subjectList = JSON.parse(data);
					var html = "<option value>请选择</option>"
					for(var i=0; i<subjectList.length; i++) {
						html+="<option value='"+subjectList[i].code+"'>"+subjectList[i].name + "</option>";
					}
	...
	function getGradeCode(gradeCode) {
		var code = $("#stageCode").val();
		if(code != "") {
			$.gradeCodeSelector("#gradeCode",{"tn" : "jc_grade"}, '${jcTextbook.gradeCode}', code, function(data) {
				return {"val" : data.code, "title" : data.name};
			}, function() {
