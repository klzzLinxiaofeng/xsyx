...
function sure(){
	...
	loader.show();
	$.post("${pageContext.request.contextPath}/teach/incentiveEvaluation/bbxx", $requestData, function(data, status) {
		if("success" === status) {
			$("#kb_tb").html(data);
		}
		loader.close();
...
function myself(){
	...
	if(teamId != null && "" != teamId){
		$.post("${pageContext.request.contextPath}/teach/incentiveEvaluation/myself", $requestData, function(data, status) {
			if("success" === status) {
				$("#kb_tb").html(data);
			}
			loader.close();
