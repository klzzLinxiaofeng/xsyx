...
<body>
<h3>题目类型 得分率统计</h3>
<svg id="initCircle_tmlx" width="390" height="416" font-family="sans-serif" font-size="10" text-anchor="middle" style="margin:0 auto;display:block;"></svg>
...
<script>
    ...
    function getDateForD3_timulx(){
        $.post("${ctp}/statistic/teachingEvaluationQuestType", {"examId":${exam.id}}, function (returnData, status) {
            if ("success" === status) {
                ...
                if(returnData != ""){
                    $.each(returnData,function(n,value) {
                        if(typeof(value.percent) == "number"){
                            ...
                            if(p > 0){
                                if(classes == ""){
                                    classes = "{\"name\":\""+value.type+"\",\"value\":\""+(p)+"\"}";
                                }else{
                                    classes += ",{\"name\":\""+value.type+"\",\"value\":\""+(p)+"\"}";
                                }
                            }
                    ...
                    if(classes == ""){
                        $("#initCircle_tmlx").hide();
                    }
                    initCircle_tmlx(JSON.parse("["+classes+"]"));
                }
            }
        });
    }

    function initCircle_tmlx(data){

        var width  = 590;	//SVG绘制区域的宽度
        var height = 461;	//SVG绘制区域的高度

        d3.select("#initCircle_tmlx").selectAll("g").remove();//清空作图区域
        var svg = d3.select("#initCircle_tmlx"),
