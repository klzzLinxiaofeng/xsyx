...
<body>
	<div class="container-fluid">
		<jsp:include page="/views/embedded/navigation.jsp">
			...
			<jsp:param value="走班课程" name="title" />
...
<!-- 											<th>总学时</th> -->
								...
								<tbody id="bwSubject_list_content">
									<jsp:include page="./list.jsp" />
								</tbody>
							</table>
							<jsp:include page="/views/embedded/jqpagination.jsp" flush="true">
								<jsp:param name="id" value="bwSubject_list_content" />
								<jsp:param name="url" value="/bbx/bwSubject/index?sub=list&dm=${param.dm}" />
...
$(function() {
	...
	function search() {
		...
		var id = "bwSubject_list_content";
		var url = "/bbx/bwSubject/index?sub=list&dm=${param.dm}";
	...
	function loadCreatePage() {
		...
		$.initWinOnTopFromLeft('创建', '${ctp}/bbx/bwSubject/creator?gradeId=' + gradeId, '700', '400');
	}
	//  加载编辑对话框
	function loadEditPage(id) {
		$.initWinOnTopFromLeft('编辑', '${ctp}/bbx/bwSubject/editor?id=' + id, '700', '300');
	}
	
	function loadViewerPage(id) {
		$.initWinOnTopFromLeft('详情', '${ctp}/bbx/bwSubject/viewer?id=' + id, '700', '300');
	}
	...
	function executeDel(obj, id) {
		$.post("${ctp}/bbx/bwSubject/" + id, {"_method" : "delete"}, function(data, status) {
			if ("success" === status) {
				if ("success" === data) {
					$.success("删除成功");
					$("#" + id + "_tr").remove();
