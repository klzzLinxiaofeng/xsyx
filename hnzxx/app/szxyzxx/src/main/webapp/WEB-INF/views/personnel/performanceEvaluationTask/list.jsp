...
<c:forEach items="${voList}" var="vo" varStatus="i">
	<tr id="${vo.id}_tr">
		...
		<td class="caozuo">
			<c:choose>
				<c:when test="${vo.isIssue == false }">
					<c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 2)}">
					<button class="btn btn-blue" type="button" onclick="issueTask(this, '${vo.id}');">发布</button>
					</c:if>
				</c:when>
				<c:otherwise>
					<c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 2)}">
					<button class="btn btn-blue" type="button">已发布</button>
					</c:if>
				</c:otherwise>
			</c:choose>
			...
			<c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 3)}">
				<button class="btn btn-red" type="button" onclick="deleteObj(this, '${vo.id}');">删除</button>
			</c:if>
		</td>
	</tr>
</c:forEach>
