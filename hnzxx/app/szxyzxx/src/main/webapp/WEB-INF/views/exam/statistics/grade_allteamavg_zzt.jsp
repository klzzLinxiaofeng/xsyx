...
<script type="text/javascript">
    ...
    function createUILd(data1,data2){
        //全年级平均分 雷达对比图
        var myChart = echarts.init(document.getElementById('radar2567'));
        option = {
            title: {
                text: ''
            },
    ...
    function getDataLd(){
        $.post("${ctp}/statistic/gradeAvgLd",{"examId":${exam.id}}, function(returnData, status) {
            if ("success" === status) {
                ...
                var value2 = new Array(returnData.length);
                $.each(returnData,function(n,value) {
                    if(data1 == ""){
                        data1 = "{\"text\":\""+value.teamName+"\",\"max\":\""+returnData.length+"\"}";
                    }else{
                        data1 += ",{\"text\":\""+value.teamName+"\",\"max\":\""+returnData.length+"\"}";
                    }
                    value2[n] = value.sdScore;
                    teamAvg[n] = value.averageScore;
                });

                json = "["+data1+"]";
                var teamAvgArr = countBZC(teamAvg);
                var data2 = "[{\"name\":\"年级标准差\",\"value\":[" + getArr2Grade(returnData.length,teamAvgArr) + "]},{\"name\":\"班级标准差\",\"value\":["+value2+"]}]";
