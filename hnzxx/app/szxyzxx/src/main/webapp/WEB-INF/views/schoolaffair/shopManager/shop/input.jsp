				...
				<div style="padding: 20px 0 0;" class="widget-container">
					<form class="form-horizontal" id="canteen_form"
						...
						<div class="control-group">
							<label class="control-label"><font style="color: red">*</font>超市编号</label>
							<div class="controls">
								<input type="text" id="code" name="code" class="span4"
									placeholder="请输入超市编号, 不能为空" <c:if test="${not empty shop.code}">disabled="disabled"</c:if> value="${shop.code}"
									${isCK != null && isCk != '' ? "disabled='disabled'" : ""}>
							</div>
						</div>
						
						<div class="control-group">
							<label class="control-label"><font style="color: red">*</font>超市名称</label>
							<div class="controls">
								<input type="text" id="name" name="name" class="span4"
									placeholder="请输入超市名称, 不能为空" value="${shop.name}"
									${isCK != null && isCk != '' ? "disabled='disabled'" : ""}>
							</div>
						</div>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="leader" name="leader" class="span4"
									placeholder="请输入负责人, 不能为空" value="${shop.leader}"
									${isCK != null && isCk != '' ? "disabled='disabled'" : ""}>
							</div>
						</div>
						...
						<div class="control-group">
							...
							<div class="controls">
								<input type="text" id="runTime" name="runTime" class="span4"
									placeholder="样式：周一到周五-几点到几点" value="${shop.runTime}"
									${isCK != null && isCk != '' ? "disabled='disabled'" : ""}>
							</div>
						</div>
						...
						<div class="control-group">
								...
								<div class="controls">
									...
									<c:forEach items="${floorList }" var="floor">
										<option value="${floor.code}" id="${floor.isDelete }" data-id="${floor.state }" <c:if test="${shop.floorCode == floor.code }">selected</c:if>>${floor.name }</option>
									</c:forEach>
								</select>
								</div>
							</div>
							<div class="control-group">
								...
								<div class="controls">
								<input type="hidden" id="layer" value="${empty shop ? '' : shop.layerNumber}">
						...
						<div class="control-group">
							...
							<div class="controls">
								<textarea  id="remark" name="remark"
									...
									${isCK != null && isCk != '' ? "disabled='disabled'" : ""}>${shop.remark}</textarea>
							</div>
						</div>
...
// 		alert(state)
							...
							rules : {
								"code" : {
									...
									remote : {
										async : false,
										type : "GET",
										url : "${pageContext.request.contextPath}/schoolaffair/shop/checker",
										data : {
											'dxlx' : 'code',
											'code' : function() {
												return $("#code").val();
								...
								"name" : {
									...
									remote : {
										async : false,
										type : "POST",
										url : "${pageContext.request.contextPath}/schoolaffair/shop/nameChecker",
										data : {
											'dxlx' : 'name',
											'name' : function() {
												return $("#name").val();
	...
	function saveOrUpdate() {
		if (checker.form()) {
			...
			var url = "${pageContext.request.contextPath}/schoolaffair/shop/creator";
			if ("" != $id) {
				$requestData._method = "put";
				url = "${pageContext.request.contextPath}/schoolaffair/shop/" + $id;
			}
