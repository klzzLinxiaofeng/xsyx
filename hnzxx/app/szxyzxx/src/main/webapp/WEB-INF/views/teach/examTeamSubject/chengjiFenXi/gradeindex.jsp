            ...
            <div class="widget-head">
                <h3>
                    ...
                    <p class="btn_link" style="float: right;">
                        ...
                        <a id="downLoadExcel"  class="a2" href="#" onclick="daochu();" class="a2" >导出年级题目得分分析</a>
...
<script type="text/javascript">
    $(function () {
        initSelect();
        var strs='<thead><tr role="row"><th>考试名称</th><th>班级名称</th><th>班级人数</th><th>参考人数</th><th>总分</th><th>任课教师</th></tr></thead><tbody id="publicClass_list_content"></tbody>';
                    ...
                    val.gradeId = nj;
                        if(xl!=null && xl !=""){
                            ...
                            if (km != null && km != "") {
                                ...
                                var url = "/scoreFenXi/findByGradeTiMuFenXi";
                                $.post(url,val,function (d) {
                                    var dd=JSON.parse(d);
                                    var list=dd['list'];
                                    var thTop=dd['ExamList'];
                                    var strs="<thead><tr role='row'><th>考试名称</th><th>班级名称</th><th>班级人数</th><th>参考人数</th><th>总分</th>";
                                    for(var i=0;i<thTop.length;i++){
                                        strs+="<th>第"+(parseInt(i)+1)+"题</th>";
                                    }
                                    strs+="<th>任课教师</th></tr></thead><tbody id= 'publicClass_list_content'>";
                                    for(var a=0;a<list.length;a++){
                                        strs+="<tr><td>"+list[a].examName+"</td><td>"+list[a].teamName+"</td><td>"+list[a].teamSize+"</td><td>"+list[a].cankao+"</td><td>"+list[a].zongfen+"</td>";
                                    for(var j=0;j<thTop.length;j++){
                                        strs+="<td>"+list[a]['data_'+j]+"%</td>";
                                    }
                                    strs+="<td>"+list[a].teacher+"</td></tr>";
                                    }
                                    strs+="</tbody>";
                                    $("#data-table").html("");
                                    $("#data-table").append(strs);
    ...
    function daochu() {
        var url = "/scoreFenXi/daochuGradeFenXi";
        var nj = $("#nj").val();
        var xn = $("#xn").val();
        var xq = $("#xq").val();
        var km =$("#km").val();
