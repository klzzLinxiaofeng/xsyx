...
<c:forEach items="${items}" var="item" varStatus="sta">
	<tr id="${item.id}_tr">
		...
		<td><a href="javascript::void(0)" onclick="roadRuleItem('${item.id}');">${item.name}</a></td>
		<td>
		...
		<td class="caozuo">
			<c:if test="${'考核完成' != item.state}">
				<button class="btn btn-blue" type="button" onclick="loaduploadPage('${item.id}');">导入</button>
