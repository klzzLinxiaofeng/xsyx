		...
		<div class="span12">
			<div class="content-widgets" style="margin-bottom:0">
				<div class="widget-container"  style="padding:20px 0 0;">
					<form class="form-horizontal" id="subjectModify_form" action="javascript:void(0);">
						<input type="hidden" name="id" id="id" value="${subject.id }"/>
<!-- 						<div class="control-group"> -->
<!-- 							<label class="control-label"><font style="color:red">*</font>学段</label> -->
<!-- 							<div class="controls"> -->
<!-- 								<select id="stageCode" id="stageCode" name="stageCode" onchange="getStageName(this);" class="span4"> -->
<%-- 			                           	<c:forEach items="${sessionScope[sca:currentUserKey()].stageCodes}" var="stage"> --%>
<%-- 			                           		<option value="${stage}" <c:if test="${stage==subject.stageCode }">selected</c:if> > --%>
...
<!-- 							<div class="controls"> -->
<!-- 								<select  name="publicSubject" id="publicSubject" class="span4" onchange="getPublicSubject(this);"> -->
<!--  								</select> -->
<!-- 							</div> -->
<!-- 						</div> -->
						...
						<div class="control-group" id="locationSubjectId">
							...
							<div class="controls">
								<input type="text" name="name" id="name" value="${subject.name }" class="span4"/>
							</div>
						</div>
...
<script type="text/javascript">
	...
	$(function () {
		//课程范围
		$.jcGcSelector("#subjectClass", {tc : "JY-KCFW"}, "${subject.subjectClass}", function() {
			
		});
		//课程类别
		$.jcGcSelector("#subjectType", {tc : "JY-KCLB"}, "${subject.subjectType}", function() {
			
		});
		//课程属性
		$.jcGcSelector("#subjectProperty", {tc : "JY-KCSX"}, "${subject.subjectProperty}", function() {
			
		});
		//课程性质
		$.jcGcSelector("#subjectCharacter", {tc : "JY-KCXZ"}, "${subject.subjectCharacter}", function() {
...
// 		}
	...
	function loadTable(data){
		...
		var subCode = ${subject.code};
		
		var subjectTemp = "<option value=''>请选择</option>";
		for (var i = 0, len = data.length; i < len; i++) {
			var temp= "";
			if(subCode==data[i].code){
				subjectTemp += "<option value="+data[i].code+" selected>"+data[i].name+"</option>";
			}else{
	...
	function getSubject(obj){
		var $this = $(obj);
		var $requestData = {tn : "jc_subject"};
		$requestData.pn = "stageCode";
		$requestData.value = $this.val();
		$.jcSelector("#name", $requestData, "${subject.code}", function(data) {
			return {val: data.code, title : data.name}
		});
	}
	
	function initValidator() {
		return $("#subjectModify_form")
			.validate(
					{
						errorClass : "myerror",
						rules : {
...
// 								remote:"你没有权根选公共课程"
	...
	function saveOrUpdate() {
		if(checker.form()){
			...
			var $id = $("#id").val();
			var $requestData = formData2JSONObj("#subjectModify_form");
			var url = "${pageContext.request.contextPath}/teach/subject/updateSubject";
			loader.show();
