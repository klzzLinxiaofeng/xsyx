...
<c:forEach items="${items}" var="item">
	<tr id="${item.id}_tr">
				 <td> ${fn:length(item.title)>10 ? fn:substring(item.title,0,10) : item.title }${fn:length(item.title)>10 ? "..." : "" }</td>
				<td> ${fn:length(item.contentHtml)>50 ? fn:substring(item.contentHtml,0,50) : item.content }${fn:length(item.contentHtml)>50 ? "..." : "" }</td>
