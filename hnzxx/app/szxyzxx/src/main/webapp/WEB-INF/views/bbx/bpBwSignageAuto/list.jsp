...
<c:forEach items="${items}" var="item">
	<tr id="${item.teamId}_tr">
		...
		<td class="caozuo">
			...
			<button class="btn btn-blue" type="button" onclick="loadEditPage('${item.teamId}');">编辑</button>
<%-- 			 </c:if> --%>
<%-- 			<c:if test="${acl:hasPermission(sessionScope[sca:currentUserKey()].id, param.dm, 3)}"> --%>
				<button class="btn btn-red" type="button" onclick="deleteObj(this, '${item.teamId}');">删除</button>
<%-- 			</c:if>  --%>
		</td>
	</tr>
</c:forEach>
