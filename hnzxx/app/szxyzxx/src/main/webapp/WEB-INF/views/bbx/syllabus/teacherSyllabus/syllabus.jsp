		...
		<table width="100%" border="0" cellspacing="0" cellpadding="0" id="data-table">
			...
			<tbody>
				<c:forEach items="${lessons}" var="lesson" varStatus="status">
					<tr>
						<td data-kj="${lesson}" style="width:2% !important ;vertical-align: middle;">${status.count}</td>
						<c:forEach items="${dayOfWeek}" var="day">
...
<style>
  .row-fluid [class*="span"] .left_span{ display: none ;}
   ...
   .left_span span:hover{color:#da9f0a;cursor: pointer;}
   .left_span span{ display: inline-block;padding:0 6px;}
</style>
	<script>
		$(function(){
			var loader = new loadLayer("加载课表中");
			var $requestData = {};
			$requestData.teacherId = $("#teachId").attr("data-id");
			$.get("${ctp}/bbx/teacherSyllabus/rkap/list/json", $requestData, function(data, status) {
				if("success" === status) {
					data = eval("(" + data + ")");
					$.each(data, function(index, value) {
						...
						kbCell.html("<div style='min-height:50px;font-size:15px;'>" + subjectName  +"<em>" + roomName + "</em>" 
							+ logol + "</div>"
							+ "<div class='left_span'>" 
							+ "<span class='edit new_pj member' data-kbdm='"+kbDm+"'  ><i class='icon-edit icon-1x'></i>成员</span>"
							+ "<span class='edit new_pj evaluate' data-kbdm='"+kbDm+"'  ><i class='icon-edit icon-1x'></i>评价</span>"
							+"<span class='edit new_pj ' data-kbdm='"+kbDm+"' data-type='" + type +"'><i class='icon-edit icon-1x'></i>考勤</span></div></div>");
						} else if ( type == 1 ) {
							kbCell.html("");
							kbCell.html("<div style='min-height:50px;font-size:15px;'>" + subjectName  +"<em>" + roomName + "</em>" 
							+ logol + "</div>");
