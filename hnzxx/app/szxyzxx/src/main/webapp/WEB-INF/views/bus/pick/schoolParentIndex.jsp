...
<div class="set" onclick="getTime(),openSet()">
    <img src="${ctp}/images/设置_setting-two (1).png" alt="设置">
</div>
...
<div class="setPath" style="display: none;">
    <div class="setPathTitle">
        <span>设置播放时间</span>
        <img id="setColse" src="${ctp}/images/关闭_close.png" alt="关闭" onmousedown="quitSet();">
    </div>
    <form class="setPlayForm" action="">
        <div id="timeList">
        </div>
        <div class="btu">
            <input type="submit" value="保 存" class="submit" onmousedown="quitSet();">
            <input type="button" value="添加时间点" class="add" onmousedown="addTime();">
        </div>
    </form>
</div>
    ...
    function getStuCentont() {
        ...
        if (nowDateStr == cacheDateStr) {
            if (nowHour >= 12 && nowHour < 20) {
                var divs = ['parkIn', 'parkOut', 'gateIn', 'gateOut'];
                //测试数据
                $.get("/bus/pickData/schoolParentList?id=1", function (d) {
