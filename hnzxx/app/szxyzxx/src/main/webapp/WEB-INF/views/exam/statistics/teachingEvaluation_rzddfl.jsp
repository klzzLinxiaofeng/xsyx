...
<body>
<h3>题目认知度得分率统计</h3>
<svg id="initCircle_rzd" width="390" height="416" font-family="sans-serif" font-size="10" text-anchor="middle" style="margin:0 auto;display:block;"></svg>
...
<script>
    ...
    function getDateForD3_rzd(){
        $.post("${ctp}/statistic/teachingEvaluationCognition", {"examId":${exam.id}}, function (returnData, status) {
            if ("success" === status) {
                ...
                if(returnData != ""){
                    ...
                    initCircle_rzd(JSON.parse("["+classes+"]"));
                }
            }
        });
    }

    function initCircle_rzd(data){

        var width  = 590;	//SVG绘制区域的宽度
        var height = 461;	//SVG绘制区域的高度

        d3.select("#initCircle_rzd").selectAll("g").remove();//清空作图区域
        var svg = d3.select("#initCircle_rzd"),
