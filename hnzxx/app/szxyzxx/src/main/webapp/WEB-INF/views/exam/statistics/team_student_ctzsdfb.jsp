...
<body>
<h3>错题知识点分布统计</h3>
<svg id="initCircleMine" width="390" height="416" font-family="sans-serif" font-size="10" text-anchor="middle" style="margin:0 auto;display:block;"></svg>
...
<script>
    ...
    function getDateForD3Mine(){
        $.post("${ctp}/statistic/getTeamStudentCTZSDFB", {"examId":${exam.id},"studentId":$("#stu").val()}, function (returnData, status) {
            if ("success" === status) {
                ...
                if(returnData != ""){
                    $.each(returnData,function(n,value) {
                        if(typeof(value.scoreCount) == "number"){
                            var p = Number(100 - (value.scoreCount)).toFixed(2);
                            if(p > 0){
                                if(classes == ""){
                    ...
                    if(classes == ""){
                        ...
                        $("#initCircleMine").show();
                        initCircleMine(JSON.parse("["+classes+"]"));
                    }
                }
            }
        });
    ...
    function initCircleMine(data){
        var width  = 380;	//SVG绘制区域的宽度
        var height = 461;	//SVG绘制区域的高度

        d3.select("#initCircleMine").selectAll("g").remove();//清空作图区域
        var svg = d3.select("#initCircleMine"),
...
//       var color = d3.scaleOrdinal(d3.schemeCategory20c);
        ...
        node.append("text")
                ...
                .data(function(d) {
                    var arr = new Array();
                    arr.push(d.class);
                    arr.push(Number(d.data.value).toFixed(2) +"%");
                    return arr;//圆内显示内容
                })
                .enter().append("tspan")
                .attr("x", 0)
