					...
					<div class="content-widgets" style="margin-bottom:0;">
						<div class="widget-container">
							<div class="select_b">
								...
								<div class="select_div">
									<input type="hidden" id="xn" name="xn" value="${schoolYear}"/>
									<input type="hidden" id="xq" name="xq" value="${termCode}"/>
									<input type="hidden" id="nj" name="nj"/>
									<span style="padding-left: 30px;">班级： </span>
									<select id="bj" name="bj"  class="chzn-select" style="width: 120px;" onchange="changeTeam"></select>							
								</div>					
								<button class="btn btn-primary" type="button"
									onClick="getSyllabus();">开始设置</button>
								<button class="btn btn-warning" type="button"
									onClick="pushSySyllabus();">同步推送课程表</button>
								<div class="clear"></div>
							</div>
						</div>
					</div>
...
<script type="text/javascript">
	$(function() {
		...
		var $requestData = {"roleType" : "CLASS_MASTER"};
	...
	function changeTeam(){
		...
		if(bj != ""){
			$.post(
				"${pageContext.request.contextPath}/teach/syllabus/getGrade", 
				{teamId: bj}, 
...
// 		$(obj).addClass("active");
	...
	function intlessonInput() {
		...
		var moringMaxLesson = "${syllabus.lessonOfMorning}";
		if(moringMaxLesson != ""){
		...
		var afternoonMaxLesson = "${syllabus.afternoonOfMorning}";
		...
		var eveningMaxLesson = "${syllabus.eveningOfMorning}";
		if(eveningMaxLesson != ""){
	...
	function pushSySyllabus(){
		...
		if(teamId != "" && termCode != ""){			
			if(confirm("是否发送同步课程表推送")==true){
				...
				loader.show();
				$.post(
					"${pageContext.request.contextPath}/teach/syllabus/pushSySyllabus", 
					{teamId: teamId, termCode: termCode}, 
					function(data, status) {	
						if("success" === status) {
							$.success('操作成功');
							data = eval("(" + data + ")");
