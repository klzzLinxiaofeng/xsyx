	...
	<div id="apply_${item.id}">
		...
		<div class="clearfix x-mright">
			<div class="x-load span12">
				<span>
					<em class="on" onclick="sqRepari('${item.id}');" style="cursor:pointer">
						申请
					</em>
				</span>
				<span <c:if test="${(item.status==04 || item.status==03) && (item.appraise==null || item.appraise=='')}">class="active-on"</c:if>>
					<em <c:if test="${item.status!=01}">class="on" onclick="wxRepari('${item.id}');" style="cursor:pointer"</c:if> >
						维修
					</em>
				</span>
				<span>
					<em <c:if test="${item.appraise!=null && item.appraise!=''}">class="on"</c:if>
					 <c:if test="${item.status!=02 && item.status!=01}">onclick="pjRepari('${item.id}');" style="cursor:pointer"</c:if> >
					 	评价
					</em>
				</span>
...
<jsp:include page="./myJqpagination.jsp" flush="true">
	...
	<jsp:param name="url" value="/oa/applyrepair/index?sub=list&own=${ownType}&dm=${param.dm}" />
