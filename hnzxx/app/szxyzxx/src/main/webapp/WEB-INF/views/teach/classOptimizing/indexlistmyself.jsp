...
<span id="name"></span> 课堂优化状况数据分析</span>
...
function search(){
	...
	if(teamId != null && "" != teamId){
		$.post("${pageContext.request.contextPath}/teach/classOptimizing/inmyself", $requestData, function(data, status) {
			if("success" === status) {
				$(".data").html(data);
			}
			loader.close();
...
function searchmyself(studentName,studentId){
	...
	loader.show();
	if(teamId != null && "" != teamId){
		$.post("${pageContext.request.contextPath}/teach/classOptimizing/myselfList", $requestData, function(data, status) {
			if("success" === status) {
				$(".data").html(data);
			}
			loader.close();
