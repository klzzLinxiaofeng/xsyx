...
<script type="text/javascript">
	(function($){
		...
		//在当前的窗口左侧显示窗口 title:窗口标题；url：加载的页面url；width：窗口宽度；height：窗口高度；callback：关闭时回调
		$.initWinHasCloseCallback = function(title, url, width, height, callback) {
			window.top.layer.open({
				skin: 'layui-layer-qyjx', //样式类名
			    type: 2,
			    title: title,
...
// 			loadi = topLayer.load("加载中...", 120);
			loadi = topLayer.load(0,{time: 120*1000});
    ...
    function load_xq(msg,time) {
        var topLayer = top != null? top.layer : layer;
        var loadi;
        this.show = function() {
// 			loadi = topLayer.load("加载中...", 120);
            loadi = topLayer.load(3,{time: 120*1000});
            //loadi = topLayer.msg('加载中...', {icon: 16,shade: [0.5, '#f5f5f5']});
        };
        this.close = function() {
            topLayer.close(loadi);
