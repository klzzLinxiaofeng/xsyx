...
<script type="text/javascript">
	...
	//根据校本教材目录查询条件查找对应的目录
	function resTextBookCatalogListByCondition(stageCode, subjectCode, gradeCode,
			publisherId, volumn, functionName, div, prefixId) {
		resTextBookCatalogList(div, stageCode, gradeCode, subjectCode, volumn,
				publisherId, 'select', '', functionName, prefixId);
	}
	...
	function textBookCatalogList(div, stageCode, gradeCode, subjectCode,
		...
		$.ajax({
				...
				success : function(data) {
					...
					if(jsonObj.length == 1) {
						var $allli = $("<li id = '"+prefixId+"all'><a href='javaScript:void(0);' title='全部'><b onclick='"
								+ functionName
								+ "(-1, this)'></b></a></li>");
						
						$("#" + prefixId + "ulall").append($allli);
						var aa = $allli.find("b").length;
						//默认选中全部
					 	$allli.find("b").click();
					 	return false;
					}
					...
					$.each(
						...
						function(i, item) {
							if (item.level == 0) {
								...
								var $uraddli = "<li  class='lesson-item-child"+item.level+"' id="+item.id+"><a title='"
								+ item.name
	...
	function resTextBookCatalogList(div, stageCode, gradeCode, subjectCode,
			volumn, publisherId, type, catalogId, functionName, prefixId) {
		//var functionName = '_dblclick';
		//alert("div:"+div+",stageCode:"+stageCode+",gradeCode:"+gradeCode+",subjectCode："+subjectCode+",volumn："+volumn+",publisherId:"+publisherId+",type:"+type+",catalogId:"+catalogId);
		$("#" + div).empty();
		...
		$.ajax({
				type : "post",
				url : "${pageContext.request.contextPath}/teach/textBookMaster/master/resTextBookCatalogList", //${ctp}/teach/textBookMaster/master
				data : {
					'stageCode' : stageCode,
					'subjectCode' : subjectCode,
					'gradeCode' : gradeCode,
					...
					$.each(
						...
						function(i, item) {
							if (item.level == 0) {
								...
								var $unitaddli = "<li class='unit-item' id="+prefixId+"li"+unitAddLi+"><a href='javaScript:void(0);' class='unit-link strong ib' title='"
										+ item.name
										+ "'><b onclick='"+functionName+"(\""+item.code+"\",this);'>"
										+ item.name
										+ "</b></a>";
								var ul = prefixId + "ul"
										+ unitAddLi;
								...
								var $uraddli = "<li  class='lesson-item-child"+item.level+"' id="+item.id+"><a title='"
								+ item.name
								+ "' href='javaScript:void(0);' class='lesson-link ib pl5 pt5'><b onclick='"+functionName+"(\""+item.code+"\",this);'>"
								+ item.name
								+ "</b></a></li>";
								var $ul = "#" + prefixId + "ul"
										+ unitAddLi;
