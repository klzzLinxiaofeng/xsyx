...
<%@ page import="platform.szxyzxx.web.common.contants.SysCacheHttpUrlAccessor" %>
...
(function($){
	...
	$.resSelector = function (selector, $requestData, selectedVal, handler, afterHandler) {
		$.get("${ctp}<%= SysCacheHttpUrlAccessor.CACHE_JC_FINDBYPARAM_URL%>", $requestData, function(data, status) {
			if("success" === status) {
				data = eval("(" + data + ")");
				var currentSelect = $(selector);
	...
	$.stageCodeSelector = function (selector, $requestData, selectedVal, handler, afterHandler) {
		$.get("${ctp}<%= SysCacheHttpUrlAccessor.CACHE_JC_FINDBYPARAM_URL%>", $requestData, function(data, status) {
			if("success" === status) {
				data = eval("(" + data + ")");
				var currentSelect = $(selector);
				...
				$.each(data, function(index, value) {
					var optionData = handler(value);
					var stageCodes = "${stageCode}";
					if(stageCodes.indexOf(optionData.val)!=-1) {
						currentSelect.append("<option value='" + optionData.val + "'>" + optionData.title + "</option>");
					}
				});
				currentSelect.val(selectedVal);
	...
	$.subjectCodeSelector = function (selector, $requestData, selectedVal, subjectCode, handler, afterHandler) {
		$.get("${ctp}<%= SysCacheHttpUrlAccessor.CACHE_JC_FINDBYPARAM_URL%>", $requestData, function(data, status) {
			if("success" === status) {
				data = eval("(" + data + ")");
				var currentSelect = $(selector);
				...
				$.each(data, function(index, value) {
					var optionData = handler(value);
					if( (value.stageCode).indexOf(subjectCode)!=-1 ) {
						currentSelect.append("<option value='" + optionData.val + "'>" + optionData.title + "</option>");
					}
				});
				currentSelect.val(selectedVal);
	...
	$.gradeCodeSelector = function (selector, $requestData, selectedVal, stageCode, handler, afterHandler) {
		$.get("${ctp}<%= SysCacheHttpUrlAccessor.CACHE_JC_FINDBYPARAM_URL%>", $requestData, function(data, status) {
			if("success" === status) {
				data = eval("(" + data + ")");
				var currentSelect = $(selector);
				...
				$.each(data, function(index, value) {
					var optionData = handler(value);
					if( (value.stageCode).indexOf(stageCode)!=-1 ) {
						currentSelect.append("<option value='" + optionData.val + "'>" + optionData.title + "</option>");
					}
				});
				currentSelect.val(selectedVal);
