...
//	console.log(appGetTermCode+"////"+appGetTeamId+"////"+appGetBeginDate+"////"+appGetEndDate);
	if(urlForTableList != null){
		$.ajax({  
			...
			success : function(data){   //console.log("班级1111"+JSON.stringify(data));
			var data=data.data;
			var napededuct=0;
			var napeawarded=0;
			var napepercent=0;
...
//	termCode = $(".year").attr("data-termCode");
	...
	if($("#select_div_week").is(":hidden")){
		...
		$.ajax({  
			...
			success : function(data){   //console.log("班级22222"+JSON.stringify(data));
				var data=data.data;
				$(".table-list").html("");
				if(data!=null && data.length>0){
					for(var i=0;i<data.length;i++){
	...
	if($("#select_div_week").is(":hidden")){
		if(c_name == 1){
			...
			$.ajax({  
				...
				success : function(data){   //console.log("年级222"+JSON.stringify(data));
				$(".table-list").html("")
				var data=data.data;
				if(data!=null && data.length>0){
					for(var i=0;i<data.length;i++){
					...
					success : function(data){   //console.log("年级"+JSON.stringify(data));
						...
						if(data!=null && data.length>0){
							for(var i=0;i<data.length;i++){
								...
								napededuct += parseInt(data[i].deductScore);
								napeawarded += parseInt(data[i].addScore);
								naperatio += data[i].deductRatio;
							}
							gradeTotal = napededuct + napeawarded;
							$(".details_awarded").html("总分"+parseFloat(gradeTotal).toFixed(1));
							$(".nape_deduct").html("扣分项"+parseFloat(napededuct).toFixed(1));
							$(".nape_awarded").html("加分项+"+parseFloat(napeawarded).toFixed(1));
							loader.close();
						}else{
