(function($){
	...
	/**
	 * 上段代码，只是做了页面level等级的控制
	 * 本段代码做数据表level等级的控制
	 * 2016-10-12
	 */
	
	function createSelectByExpr(selector, val, lv,isSelected,id, callback) {
		if(val == null || "" === val) {
//			selector.html("");
//			selector.append("<option value=''>请选择</option>");
//			var s_parent=selector.parent();
//			s_parent.children().hide();
//			for(var i=0;i<lv;i++){
//				s_parent.children().eq(i).show();
//			}
			if(lv==2){
				var s_parent=selector.parent();
				s_parent.children().hide();
				s_parent.children().eq(0).show();
			}
			selector.hide();
			selector.trigger("change");
			callback(selector);
			...
			var $requestData = {};
			$requestData.tn = "jc_region";
			$requestData.expr = "parent="+val+";level="+lv;
			$.get(ctp()+"/cache/findByExpr",$requestData, function(data, status) {
				if ("success" === status) {
					data = eval("(" + data + ")");
					selector.html("");
					selector.append("<option value=''>请选择</option>");
	...
	$.initRegionLevelSelector = function(options) {
		var defOption = {
			lv : "3",   //3 / 4
			isUseChosen : false,
			sjSelector : "sj",
		...
		if (!options.isUseChosen) {
			sjSelector.on("change", function(event) {
				var $this = $(this);
				var currentId = $this.val();
				createSelectByExpr(shijSelector, currentId, 2, options.isEchoSelected, options.shijSelectedVal, options.shijCallback);
			});
			...
			if("3" === options.lv || "4" === options.lv) {
				shijSelector.on("change", function(event) {
					var $this = $(this);
					var currentId = $this.val();
					createSelectByExpr(qxjSelector, currentId, 3, options.isEchoSelected, options.qxjSelectedVal, options.qxCallback);
				});
			}
			...
			if("4" === options.lv) {
				qxjSelector.on("change", function(event) {
					var $this = $(this);
					var currentId = $this.val();
					createSelectByExpr(jdSelector, currentId, 4, options.isEchoSelected, options.jdSelectedVal, options.jdCallback);
				});
			}
			
			createSelectByExpr(sjSelector, "0", 1, options.isEchoSelected, options.sjSelectedVal, options.sjCallback);
		} else {
			$("body").on("click", "#" + options.sjSelector + "_chzn .chzn-drop .chzn-results li", function(event) {
				var $this = $(this);
				var sjIndex = $this.attr("id").replace(options.sjSelector + "_chzn_o_", "");
				var $sjSelector = sjSelector;
				var sj = $sjSelector.find("option").eq(parseInt(sjIndex)).val();
				createSelectByExpr(shijSelector, sj, 2, options.isEchoSelected, options.shijSelectedVal, function(selector) {
					options.shijCallback(selector);
					$("#" + options.shijSelector + "_chzn").remove();
					shijSelector.show().removeClass("chzn-done").chosen();
					if(options.isEchoSelected || "" === sj || sj == null) {
			...
			if("3" === options.lv || "4" === options.lv) {
				$("body").on("click", "#" + options.shijSelector + "_chzn .chzn-drop .chzn-results li", function(event) {
					...
					var $shijSelector = shijSelector;
					var shij = $shijSelector.find("option").eq(parseInt(shijIndex)).val();
					createSelectByExpr(qxjSelector, shij, 3, options.isEchoSelected, options.qxjSelectedVal, function(selector) {
						options.qxCallback(selector);
						$("#" + options.qxjSelector + "_chzn").remove();
						qxjSelector.show().removeClass("chzn-done").chosen();
						if(options.isEchoSelected || "" === shij || shij === null) {
			...
			if("4" === options.lv) {
				$("body").on("click", "#" + options.qxjSelector + "_chzn .chzn-drop .chzn-results li", function(event) {
					...
					var $qxjSelector = qxjSelector;
					var qxj = $qxjSelector.find("option").eq(parseInt(qxjIndex)).val();
					createSelectByExpr(jdSelector, qxj, 4, options.isEchoSelected, options.jdSelectedVal, function(selector) {
						options.jdCallback(selector);
						$("#" + options.jdSelector + "_chzn").remove();
						jdSelector.show().removeClass("chzn-done").chosen();
						if(options.isEchoSelected || qxj == null || "" === qxj) {
			...
			createSelectByExpr(sjSelector, "0", 1, options.isEchoSelected, options.sjSelectedVal, function(selector) {
				options.sjCallback(selector)
				sjSelector.chosen();
				shijSelector.chosen();
				qxjSelector.chosen();
