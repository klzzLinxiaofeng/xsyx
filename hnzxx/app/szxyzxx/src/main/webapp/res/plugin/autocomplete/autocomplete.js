	...
	$.fn.autocomplete = function(params) {
		...
		this.each(function() {
			...
			var input = $('<input type="text" autocomplete="off" name="query">')
				...
				.addClass('autocomplete-input menu_name');
				/*.css({
					'width' : params.width,
			...
			input.bind("keyup", function(e){
				if(e.which != 13 && e.which != 27 
					...
					if(input.val() != ''){
                        ...
                        //console.log('开始：'+word)
						var word1=input.val();
                        var reg = new RegExp("("+word1 +")","ig");
						proposalList.empty();
                        for(var test in params.hints){
                            params.hints[test]=params.hints[test] .replace(/<b[^>]*>([^>]*)<\/b[^>]*>/ig,"$1");
                            if(params.hints[test].match(word)){
                                console.log(params.hints[test].match(word))
                                currentProposals.push(params.hints[test]);
                                params.hints[test]=params.hints[test].replace(reg,"<b style='color:#ff0000'>$1</b>");
								var element = $('<li></li>')
