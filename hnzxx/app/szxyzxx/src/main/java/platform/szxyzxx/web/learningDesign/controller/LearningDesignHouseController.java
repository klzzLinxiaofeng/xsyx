 ...
 */
@Controller
@RequestMapping(value = "/learningDesignhouse")
public class LearningDesignHouseController extends BaseController{
    ...
    private static final String DIR = "learningDesign/learningDesignhouse";
    ...
    public String delete(HttpServletRequest request, HttpServletResponse response,@CurrentUser UserInfo user) {
        ...
        Integer flag = learningDesignService.deleteMyLearningDesign(SysContants.SYSTEM_APP_ID, user.getId(), microId);
        return null;
    }
    ...
    public String edit(HttpServletRequest request, HttpServletResponse response) {
        String microId = request.getParameter("microId");
        LearningDesign ml  = this.learningDesignService.findLearningDesignByUuid(microId);
        request.setAttribute("micro", ml);
        return DIR + "/myLearningDesignEdit";
    }
    ...
    public String myLearningDesign(HttpServletRequest request, HttpServletResponse response, @ModelAttribute("page") Page page, @CurrentUser UserInfo user) {
        ...
        page.setPageSize(5);
        List<LearningDesignVo> lessonList = this.learningDesignService.searchLearningDesign(SysContants.SYSTEM_APP_ID, page, user.getId(), title, null, null, null, null);
    ...
    @RequestMapping(value = "/favLearningDesign")
    public String favLearningDesign(HttpServletRequest request, HttpServletResponse response, @ModelAttribute("page") Page page, @CurrentUser UserInfo user) {
        String index = request.getParameter("index");
        String title = request.getParameter("title");
        page.setPageSize(5);
        List<LearningDesignVo> lessonList = this.learningDesignService.searchUserFavorites(SysContants.SYSTEM_APP_ID, page,user.getId(), title, null);
        request.setAttribute("microLessonList", lessonList);
        if (index != null && !"".equals(index)) {
            return DIR + "/favLearningDesignIndex";
        } else {
            return DIR + "/favLearningDesignList";
        }
    }
    ...
    public String saveOrUpdate(HttpServletRequest request, HttpServletResponse response, @CurrentUser UserInfo user) throws IOException {
        ...
        LearningDesign ml = this.learningDesignService.saveOrUpdateLearningDesign(SysContants.SYSTEM_APP_ID, microId, entityId, LearningDesignType.COMMON_LEARNINGDESIGN, title, user.getId(), description);
        String suffix="";
        if(ml != null){
        suffix = this.entityFileService.findFileByUUID(ml.getEntityId()).getExtension();
        addPersonResource(LearningDesignCondition.DEFAULT_RESTYPE, ml.getUuid(), title, description, ml.getAppId(), null, user,suffix);
        }
        if(checked != null&&"true".equals(checked)){
        	if(ml != null){
        		this.addResource(LearningDesignCondition.DEFAULT_RESTYPE, ml.getUuid(), title, description, ml.getAppId(), null, user,suffix,vo);
        	}
