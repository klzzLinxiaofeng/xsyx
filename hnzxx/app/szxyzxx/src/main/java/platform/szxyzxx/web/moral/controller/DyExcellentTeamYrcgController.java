 ...
 */
@Controller
@RequestMapping("/dy/ex/yrcg")
public class DyExcellentTeamYrcgController {

    @Autowired
    private DyExcellentTeamYrcgService service;

    @Autowired
    private BasicSQLService basicSQLService;

    private Importer<DyExcellentTeamYrcg> importer;

    public DyExcellentTeamYrcgController(){
        ExcelImportParam<DyExcellentTeamYrcg> param=new ExcelImportParam<>(DyExcellentTeamYrcg.class);
    ...
    @RequestMapping("/list")
    @ResponseBody
    public StuVo list(Page page, DyExcellentTeamYrcg obj){
        StringBuilder sql=new StringBuilder("select * from dy_excellent_team_yrcg where xn='").append(obj.getXn()).append("' and xq='").append(obj.getXq()).append("'");
        if(obj.getXqy()!=null){
    ...
    @RequestMapping("add")
    @ResponseBody
    public BasicResult add(MultipartFile file, DyExcellentTeamYrcg obj,@CurrentUser UserInfo userInfo){
        try {
            List<Map<String,Object>> existsList=basicSQLService.find("select team_name,creator from dy_excellent_team_yrcg where of_date='"+obj.getOfDate()+"'");
            List<DyExcellentTeamYrcg> list=importer.importBy(file.getInputStream());
            Date now=new Date();
            BasicResult r= checkAndSetData(list,existsList);
            if(r!=null){
                return r;
            }
            for (DyExcellentTeamYrcg importObj : list) {
    ...
    public BasicResult delete(String ids){
        basicSQLService.update("delete from dy_excellent_team_yrcg where id in("+ids+")");
        return BasicResult.success();
    }
    
    private BasicResult checkAndSetData(List<DyExcellentTeamYrcg> list,List<Map<String,Object>> existsTeamList){
        for (DyExcellentTeamYrcg importObj : list) {
            ...
            try {
                importObj.setSumScore(importObj.getPdxk()+importObj.getRszj()+importObj.getWmlm()+importObj.getZjyx()+importObj.getYskz()+importObj.getQymb()+importObj.getWmyx()+importObj.getZj()+importObj.getGrzj()+importObj.getMckg()+importObj.getYbjc()+importObj.getDxhd()+importObj.getDkj()+importObj.getXdjsd()+importObj.getZjjsd());
    ...
    private String view(String pageName){
        return "/moral/excellentTeamYrcg/"+pageName;
    }
