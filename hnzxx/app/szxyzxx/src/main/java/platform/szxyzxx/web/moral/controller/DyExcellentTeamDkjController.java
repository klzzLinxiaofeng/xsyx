 ...
 */
@Controller
@RequestMapping("/dy/ex/dkj")
public class DyExcellentTeamDkjController {

    @Autowired
    private DyExcellentTeamDkjService service;

    @Autowired
    private BasicSQLService basicSQLService;

    private Importer<DyExcellentTeamDkj> importer;

    public DyExcellentTeamDkjController(){
        ExcelImportParam<DyExcellentTeamDkj> param=new ExcelImportParam<>(DyExcellentTeamDkj.class);
        param.setStartRowIndex(7);
    ...
    @RequestMapping("/list")
    @ResponseBody
    public StuVo list(Page page, DyExcellentTeamDkj obj,@CurrentUser UserInfo userInfo){

        StringBuilder sql=new StringBuilder("select * from dy_excellent_team_dkj where xn='").append(obj.getXn()).append("' and xq='").append(obj.getXq()).append("'");
        if(obj.getXqy()!=null){
    ...
    @RequestMapping("add")
    @ResponseBody
    public BasicResult add(MultipartFile file, DyExcellentTeamDkj obj,@CurrentUser UserInfo userInfo){
        try {
            List<Map<String,Object>> existsList=basicSQLService.find("select team_name,creator from dy_excellent_team_dkj where of_date='"+obj.getOfDate()+"'");
            List<DyExcellentTeamDkj> list=importer.importBy(file.getInputStream());
            Date now=new Date();
            BasicResult r= checkAndSetData(list,existsList);
            if(r!=null){
                return r;
            }
            for (DyExcellentTeamDkj importObj : list) {
    ...
    public BasicResult delete(String ids){
        basicSQLService.update("delete from dy_excellent_team_dkj where id in("+ids+")");
        return BasicResult.success();
    }
    
    private BasicResult checkAndSetData(List<DyExcellentTeamDkj> list,List<Map<String,Object>> existsTeamList){
        for (DyExcellentTeamDkj importObj : list) {
            ...
            try {
                importObj.setJtc(importObj.getFzty().add(importObj.getJhdl()).add(importObj.getAsdc()).add(importObj.getYxtc()).add(importObj.getCdgj()));
                importObj.setTsc(importObj.getRscjscy().add(importObj.getRscdzsm()));
                importObj.setKfpc(importObj.getJslb().add(importObj.getXsdd()).add(importObj.getPcjhdl()));
                importObj.setGxtnc(importObj.getTndzsm().add(importObj.getTnjhdl()));
                importObj.setStmjhd(importObj.getTsjscy().add(importObj.getTsxsyx()));
                importObj.setFsc(importObj.getFscdzsm().add(importObj.getFscjhdl()));
                importObj.setSumScore(importObj.getJtc().add(importObj.getTsc()).add(importObj.getKfpc()).add(importObj.getGxtnc()).add(importObj.getStmjhd()).add(importObj.getFsc()).add(importObj.getJf()));
    ...
    private String view(String pageName){
        return "/moral/excellentTeamDkj/"+pageName;
    }
