 ...
 */
@Controller
@RequestMapping("/dy/ex/xxcj")
public class DyExcellentTeamXxcjController {

    @Autowired
    private DyExcellentTeamXxcjService service;

    @Autowired
    private BasicSQLService basicSQLService;

    private Importer<DyExcellentTeamXxcj> importer;

    public DyExcellentTeamXxcjController(){
        ExcelImportParam<DyExcellentTeamXxcj> param=new ExcelImportParam<>(DyExcellentTeamXxcj.class);
    ...
    @RequestMapping("/list")
    @ResponseBody
    public StuVo list(Page page, DyExcellentTeamNjpj obj){
        StringBuilder sql=new StringBuilder("select * from dy_excellent_team_xxcj where xn='").append(obj.getXn()).append("' and xq='").append(obj.getXq()).append("'");
        if(obj.getXqy()!=null){
    ...
    @RequestMapping("add")
    @ResponseBody
    public synchronized BasicResult add(MultipartFile file, DyExcellentTeamXxcj obj,@CurrentUser UserInfo userInfo){
        try {
            List<Map<String,Object>> existsList=basicSQLService.find("select team_name,creator from dy_excellent_team_njpj where xn='"+obj.getXn()+"' and xq='"+obj.getXq()+"' and xqz='"+obj.getXqz()+"'");
            List<DyExcellentTeamXxcj> list=importer.importBy(file.getInputStream());
            Date now=new Date();
            BasicResult r=checkData(list,existsList);
            if(r!=null){
                return r;
            }
            for (DyExcellentTeamXxcj importObj : list) {
                if(importObj.getCwj()==null){
                    continue;
                }
                ...
                importObj.setSumScore(importObj.getCwj());
    ...
    public BasicResult delete(String ids){
        basicSQLService.update("delete from dy_excellent_team_xxcj where id in("+ids+")");
        return BasicResult.success();
    }
    
    private BasicResult checkData(List<DyExcellentTeamXxcj> list,List<Map<String,Object>> existsTeamList){
        for (DyExcellentTeamXxcj importObj : list) {
            if(importObj.getCwj()!=null) {
    ...
    private String view(String pageName){
        return "/moral/excellentTeamXxcj/"+pageName;
    }
