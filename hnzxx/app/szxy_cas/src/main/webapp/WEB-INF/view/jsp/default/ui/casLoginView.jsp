...
<%@ page import="com.xunyunedu.contants.SysContants" %>
<%--
...
<%--                         <label for="username" class="fl-label"><spring:message code="screen.welcome.label.netid" /></label> --%>
						...
						<input type="hidden" id="username" name="username" placeholder="用户名" value="${sessionScope.openIdLocalId}" />
<!-- 						手机号/用户名/邮箱 -->
						</c:if>

						<c:if test="${empty sessionScope.openIdLocalId}">
						<spring:message code="screen.welcome.label.netid.accesskey" var="userNameAccessKey" />
						<form:input cssClass="required" cssErrorClass="error" id="username" size="25" tabindex="1" placeholder="用户名" accesskey="${userNameAccessKey}" path="username" autocomplete="false" htmlEscape="true" />
						</c:if>
                    </div>
                    <div class="row fl-controls-left">
<%--                         <label for="password" class="fl-label"><spring:message code="screen.welcome.label.password" /></label> --%>
						...
						<form:password cssClass="required" cssErrorClass="error" id="password" size="25" tabindex="2" path="password"  placeholder="请输入您的密码" accesskey="${passwordAccessKey}" htmlEscape="true" autocomplete="off" />
                    </div>
                    <%-- <div class="row check">
                    ...
                    <div class="row btn-row">
                        ...
                        <input class="btn-submit button button-3d button-green button-rounded" name="submit" accesskey="l" value="<spring:message code="screen.welcome.button.login" />" tabindex="4" type="submit" />
...
<script type="text/javascript">
	...
	if(window != top){
		top.location.href="<%= SysContants.SZXY_BASE_PATH%>";
	}
