...
public class SzxyHttpClientRequest {
	...
	public static JSONObject doPostRequest(String url,Map<String,Object> map){
		...
		try {
			...
			String rtnJson = null;
			Header[] header = postMethod.getResponseHeaders("Content-Encoding");
	        if(header!=null&&header.length>0){
	            if("gzip".equals(header[0].getValue())){
	              rtnJson = new String(uncompress(postMethod.getResponseBody()));
	            }else{
	              rtnJson = new String(postMethod.getResponseBody(), "UTF-8");
	            }
	        }else{
	            rtnJson = new String(postMethod.getResponseBody(), "UTF-8");
	        }
